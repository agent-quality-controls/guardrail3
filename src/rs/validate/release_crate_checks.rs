...
pub fn check_per_crate(
    ...
    check_readme(pkg, krate, file, results);
    ...
    if thorough {
        release_crate_deps::check_publish_dry_run(krate, results);
    }
}
    ...
    match readme_field {
        None => {
            ...
            if default_readme.exists() {
                ...
                check_readme_quality(&default_readme, &krate.name, file, results);
            } else {
                results.push(CheckResult {
                    id: "R-PUB-04".to_owned(),
        ...
        Some(readme_path) => {
            ...
            if full_path.exists() {
                ...
                check_readme_quality(&full_path, &krate.name, file, results);
            } else {
                results.push(CheckResult {
                    id: "R-PUB-04".to_owned(),
                    severity: Severity::Warn,
...
fn check_readme_quality(
    ...
    let Some(content) = crate::fs::read_file(readme_path) else {
        return;
    };
...
mod tests {
    ...
    fn pub04_neg_no_field_no_file() {
        ...
        let mut r = Vec::new();
        check_readme(pkg_from(&t), &krate, None, &mut r);
        assert!(
            r.iter()
    ...
    fn pub05_neg_small_readme() {
        ...
        let mut r = Vec::new();
        check_readme_quality(&tmp.join("README.md"), "x", None, &mut r);
        assert!(
            r.iter()
