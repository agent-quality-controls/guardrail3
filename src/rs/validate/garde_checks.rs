...
pub fn check(workspace_root: &Path) -> Vec<CheckResult> {
    ...
    let cargo_content = crate::fs::read_file(&cargo_path);
    ...
    if let Some(clippy_content) = crate::fs::read_file(&clippy_path) {
    ...
    results.extend(check_derive_inventory(&rs_files, workspace_root));
...
// ---------------------------------------------------------------------------
// R-GARDE-02: serde deserialization method bans in clippy.toml
// ---------------------------------------------------------------------------

/// Testable: check serde method bans against clippy.toml content string.
#[cfg(test)]
...
/// Testable: check axum type bans against clippy.toml content string.
#[cfg(test)]
...
/// Testable: check `reqwest::Response::json` ban against clippy.toml content string.
#[cfg(test)]
...
fn check_derive_inventory(rs_files: &[String], workspace_root: &Path) -> Vec<CheckResult> {
