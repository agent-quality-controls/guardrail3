...
pub fn check(workspace_root: &Path, _project: &ProjectInfo, thorough: bool) -> Vec<CheckResult> {
    ...
    // Repo-level checks
    super::release_repo_checks::check_repo_level(workspace_root, &publishable_names, &mut results);

    // Binary checks
    for krate in &crates {
        if krate.is_binary {
            super::release_bin_checks::check_binary(workspace_root, krate, &mut results);
        }
    }
...
mod tests {
    ...
    fn pub12_emits_inventory() {
        ...
        let project = crate::discover::ProjectInfo {
            has_rust: true,
        ...
        let results = check(&tmp, &project, false);
        assert!(
            results
                .iter()
