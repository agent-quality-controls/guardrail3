...
## Context & Problem
The old `garde_checks.rs` implementation was only a partial baseline. It used line-based Cargo dependency detection, relied on a small stale ban list, skipped unreadable or unparsable source files silently, and only approximated derive-based validation coverage. The plan also overstated what already existed by marking `g3rs-garde/dependency-present..05` as implemented while `06..09` were still design-only and the old code had known enum and parser gaps.
...
- **Alternatives considered:**
  ...
  - Fold input failures into `g3rs-garde/dependency-present` — rejected because dependency presence and family input integrity are separate concerns.
...
- **Chose:** Resolve the nearest covering `clippy.toml` / `.clippy.toml` at allowed roots and use that for `g3rs-garde/core-method-bans/03/04/06`.
