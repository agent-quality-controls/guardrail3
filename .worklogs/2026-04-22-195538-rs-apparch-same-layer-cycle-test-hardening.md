...
Hardened the `g3rs-apparch/same-layer-cycles` test surface after the adversarial pass. The rule tests now use the real post-ingestion input shape, prove exact reported cycle members, and cover the clean acyclic branch; the dev-only ignore behavior is now proved at the ingestion pipeline boundary.

## Decisions made

- Rewrote the `g3rs-apparch/same-layer-cycles` rule helper to accept prebound same-layer edges directly.
...
- Moved the dev-only ignore assertion into the ingestion assertions crate.
  - Why: the first attempt put direct `CheckResult` field checks into a sidecar test, and `g3rs validate` correctly flagged that as `g3rs-test/assertions-modules-prove`.
...
- Continue only if another adversarial pass finds a real remaining gap in `g3rs-apparch/same-layer-cycles`.
