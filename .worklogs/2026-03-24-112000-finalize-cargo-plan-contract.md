...
- `g3rs-cargo/disallowed-macros-deny` was scoped as a policy-root rule even though resolver is a workspace concern
...
- **Chose:** Set `g3rs-cargo/lint-levels` so weaker-than-baseline levels are errors, while stricter-than-baseline settings are accepted silently.
...
- **Chose:** Convert the old mixed `g3rs-cargo/priority-order` concept into:
  - `g3rs-cargo/priority-order` for edition policy
...
- **Chose:** Make `g3rs-cargo/disallowed-macros-deny` require explicit `resolver = "2"` or `"3"` for every workspace root, with standalone packages out of scope.
...
- **Chose:** Freeze:
  - `g3rs-cargo/approved-allow-inventory` as `Error`
...
3. After the ownership rewrite, split the current mixed metadata implementation into `g3rs-cargo/priority-order` and `RS-CARGO-15`.
