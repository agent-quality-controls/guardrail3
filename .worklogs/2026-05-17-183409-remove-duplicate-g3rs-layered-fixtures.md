...
Removed the old broad `behavior/fixtures/g3rs` fixture suite after family-specific `g3rs-rules` fixtures replaced its rule coverage.

The active fixture3 suites now cover G3RS through `g3rs-rule-fixtures`, `g3rs-validate-repo`, and `g3rs-cli-output`. The old `g3rs-validate` suite is no longer in `fixture3.yaml`.
...
- Kept `g3rs-validate-repo`, `g3rs-cli-output`, and `g3rs-rule-fixtures`.
...
- `behavior/fixtures/g3rs-rules`: 1220 files, 33742 lines.
...
- If we want to remove `behavior/golden/g3rs-validate`, first migrate `behavior/migration/g3rs-test-fixture-ledger.toml` rows from old layered fixture IDs to current `g3rs-rules` fixture IDs.
