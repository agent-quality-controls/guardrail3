...
The L30 baseline set now proves missing toolchain, fmt, cargo lint tables, clippy config, deny config, service lockfile, release files, mutants config, mutants profile, and mutation hook wiring.

# Decisions Made

- Split L30 into fixtures because one fixture cannot expose base config, release, mutation-config, and mutation-profile branches without hiding required findings.
