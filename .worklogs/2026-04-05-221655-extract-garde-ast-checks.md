...
**Scope:** `packages/g3-garde-ast-checks`, `apps/guardrail3/crates/app/rs/families/garde/crates/runtime`, `apps/guardrail3/crates/app/rs/families/garde/crates/assertions`, `.plans/2026-04-04-142819-family-checks-packages.md`

## Summary
Built `g3-garde-ast-checks` for the remaining source-analysis garde rules and rewired the app garde family to call it. The app keeps malformed-input ownership in `RS-GARDE-10`, while the new package owns AST parsing and the source-rule checks for `RS-GARDE-05/07/08/09/11/12/13/14`.

## Context & Problem
`g3-garde-content-checks` already owned the root-policy config checks, but the remaining garde rules still lived in the app runtime and depended on source parsing, cross-file type knowledge, and `query_as!` inventory. The user explicitly rejected oversized orchestrator-derived fact bags as package inputs and also rejected path/root metadata garbage in the package contract. The workable direction was a second garde package that receives explicit files, reads and parses them itself, and emits the AST-related checks.

## Decisions Made

### Built a separate AST package with explicit file inputs
- **Chose:** `g3-garde-ast-checks` with one input type:
  - `G3AstFile { rel_path, abs_path }`
  - `G3GardeAstChecksInput { source_files, guardrail_toml }`
- **Why:** The AST package needs real files it is allowed to analyze, not precomputed facts and not root/mapper metadata. `guardrail3.toml` was made required because `RS-GARDE-09` needs it and the family will need it anyway.
...
- **Chose:** Package ownership for `RS-GARDE-05`, `07`, `08`, `09`, `11`, `12`, `13`, `14`.
- **Why:** These rules all operate on governed Rust source files, and `RS-GARDE-09` additionally reads `guardrail3.toml`. Cross-file checks such as nested validated types are handled internally by analyzing the whole `source_files` set inside the package.
- **Alternatives considered:**
  ...
  - Leaving `RS-GARDE-09` in the app — rejected because once `guardrail3.toml` became a required input, the same package could own it honestly.
...
- **Why:** The content/AST packages should assume valid inputs. The app already owns source-read failures, source-parse failures, and config-parse failures. Without the skip, malformed `guardrail3.toml` could double-fire with `RS-GARDE-09`.
- **Alternatives considered:**
...
- `g3-garde-content-checks` owns parsed-config rules over root `Cargo.toml` and covering `clippy.toml`.
- `g3-garde-ast-checks` owns source-analysis rules over governed Rust files plus required `guardrail3.toml`.
...
## Information Sources
- `packages/g3-garde-content-checks` — existing garde config-package specimen and current garde split.
...
- `RS-GARDE-09` still depends on legacy `guardrail3.toml`. If the family later migrates fully to `guardrail3-rs.toml`, the AST package input and internal parser calls will need to switch with the family.
...
- `packages/g3-garde-ast-checks/crates/types/src/lib.rs` — public AST package contract.
- `packages/g3-garde-ast-checks/crates/runtime/src/run.rs` — package orchestration over source files and guardrail policy.
- `packages/g3-garde-ast-checks/crates/runtime/src/support.rs` — shared AST analysis and cross-file indexing helpers.
- `packages/g3-garde-ast-checks/crates/runtime/src/parse/mod.rs` — source parsing entrypoint reused from the old family implementation.
...
1. Run package-level `arch` / `code` validation on `packages/g3-garde-ast-checks` and record any package-specific debt in `packages/g3-garde-ast-checks/TODO.md`.
