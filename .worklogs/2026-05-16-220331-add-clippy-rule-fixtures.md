...
- Split `g3rs-rule` out of `g3rs-validate` because the broad suite golden was already within 3710 bytes of the repository file-size limit.
- Kept broad layered fixtures in `g3rs-validate`; moved only `behavior/fixtures/g3rs-rule/**` into the new suite.
...
- Keep new family fixtures in `g3rs-rule`; do not add them back to the broad `g3rs-validate` suite.
