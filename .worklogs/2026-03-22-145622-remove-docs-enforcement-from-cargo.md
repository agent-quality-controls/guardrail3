...
- **Chose:** Edit `.plans/todo/checks/rs/cargo.md` so `RS-CARGO-01` says library profile adds only `unreachable_pub = "deny"` and explicitly notes that `missing_docs` is not enforced.
...
- `.plans/todo/checks/rs/cargo.md` — stale policy wording for `RS-CARGO-01`
...
- `apps/guardrail3/crates/app/rs/checks/rs/cargo/rs_cargo_01_workspace_lints.rs` — rule consuming the expectation table
