...
- Strengthened `verify-family-rule-fixtures.py` so non-clean family-rule fixtures must make every `target_rules` entry emit `Error` or `Warn`, not merely appear as `Info` inventory.
- Kept `cargo-R00-clean-golden` as the only clean Cargo fixture.

Key files for context:
- `behavior/fixtures/g3rs-rules/cargo/cargo-R30-structure-and-input-violations/fixture.toml`
