...
**Scope:** packages/g3rs-deps-config-checks; apps/guardrail3 deps family facts/run/tests
...
- Current content package support logic in `packages/g3rs-deps-config-checks/crates/runtime/src/support.rs`
...
- `g3rs-deps-config-checks` still has the known oversized-input and sibling-directory complexity debt that was explicitly deferred earlier.

## Key Files for Context
- `packages/g3rs-deps-config-checks/crates/types/src/input.rs` — current public package contract for deps content checks
- `packages/g3rs-deps-config-checks/crates/runtime/src/support.rs` — dependency identity resolution, including local path Cargo handling
