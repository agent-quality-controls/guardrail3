...
## Summary
Updated the family README so it reflects the actual enforced policy, self-hosting semantics, and current verification path. In the same batch, fixed a cross-rule inconsistency for valid rustup toolchain names with host suffixes: `g3rs-toolchain/channel-and-components` and `g3rs-toolchain/msrv-consistency` now agree that pinned stable channels such as `1.85.0-x86_64-unknown-linux-gnu` are acceptable and still participate in MSRV comparison.
...
- `g3rs-toolchain/channel-and-components` had been hardened to accept stable and pinned-stable channel names with host suffixes.
- `g3rs-toolchain/msrv-consistency` still parsed versions too narrowly and silently dropped MSRV comparison for the same accepted pinned-stable forms.
...
- **Chose:** Keep `g3rs-toolchain/channel-and-components` acceptance for host-suffixed stable names and update `g3rs-toolchain/msrv-consistency` version parsing so it strips only the host suffix after first classifying the channel family.
...
- **Alternatives considered:**
  ...
  - Keep accepting them in `g3rs-toolchain/channel-and-components` but skip `g3rs-toolchain/msrv-consistency` — rejected because it leaves the inter-rule inconsistency in place.
...
- `apps/guardrail3/crates/app/rs/families/toolchain/crates/runtime/src/rs_toolchain_config_01_channel_components_tests/mod.rs` — host-suffix acceptance coverage for `g3rs-toolchain/channel-and-components`
