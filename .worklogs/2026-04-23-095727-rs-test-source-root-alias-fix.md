...
- `RS-TEST-SOURCE-07` and `RS-TEST-SOURCE-17` now both classify that import shape through one normalization step instead of duplicating `self as` handling.
...
- Kept the runtime change small: `RS-TEST-SOURCE-17` already delegates to `has_owned_assertion_proof`, so its new regression is satisfied by the shared normalization fix plus coverage.
...
Next steps
- Land the separate `RS-TEST-FILETREE-03` import-alias chain fix already validated in `g3rs-test-file-tree-checks`.
