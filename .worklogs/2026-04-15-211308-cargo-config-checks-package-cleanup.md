...
- Added `crates/test_support` only for generic builders and parsing helpers. Rejected canned fixture exports after `g3rs-test/test-support-generic` correctly showed that hardcoded string fixtures do not belong in shared test support.
...
- Kept proof in the shared assertions crate, but used the existing `define_result_assertions!` helpers instead of inventing new local wrappers. The remaining false negatives were fixed in `g3rs-test/real-proof-site`, not by bending the package.
