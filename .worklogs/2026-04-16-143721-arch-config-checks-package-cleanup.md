Summary
- Cleaned `packages/rs/arch/g3rs-arch-config-checks` until `validate` returned `No findings.` and the package workspace tests passed.
...
- `packages/rs/arch/g3rs-arch-config-checks/crates/runtime/src/rs_arch_05_no_boundary_crossing_tests/cases.rs`
...
- Reuse this package as the reference shape for arch config-check workspaces with flat rule files, `x_tests` sidecars, and per-rule shared assertions modules.
