# Summary
Fixed the remaining decided clippy package issues by making the assertions helper `Finding` opaque and marking the config-checks workspace unpublished. The package no longer trips `g3rs-code/ast-31-public-struct-named-fields`, and the release family no longer reports publish dry-run failures for this workspace.

# Decisions Made
- Made `crates/assertions/src/common.rs::Finding` fields private instead of weakening `g3rs-code/ast-31-public-struct-named-fields`. Rejected a rule relaxation because this crate already had constructor and assertion helpers, so an opaque DTO strengthens the API without adding complexity.
...
# Next Steps
- Fix `g3rs-arch/mod-facade-only` by making rule and test sidecar `mod.rs` files pure facades.
- Continue ignoring the `test` family slice until its policy is brought back into scope.
- Decide whether the duplicated `g3rs-code/exception-comment-inventory` inventory warnings on `deny.toml` exception comments are acceptable or should be deduplicated.
