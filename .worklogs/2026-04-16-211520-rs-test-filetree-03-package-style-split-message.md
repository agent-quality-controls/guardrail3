Summary
- Updated g3rs-test/runtime-assertions-split so the missing-assertions error explains the real package-shape mistake instead of telling users to add a vague sibling assertions crate.
...
Decisions made
- Kept this as a test-family bug fix, not an apparch change, because the bad guidance starts in g3rs-test/runtime-assertions-split.
...
- After that, add a dedicated g3rs-test/runtime-assertions-split case that tells users to move it to `component/crates/assertions` and `component/crates/runtime`.
