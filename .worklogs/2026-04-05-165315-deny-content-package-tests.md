...
- **Chose:** replace flat rule files like `rs_deny_config_01_deprecated_advisories.rs` with `rs_deny_config_01_deprecated_advisories/mod.rs` + `rule.rs` plus sidecar test directories.
...
- **Chose:** populate `packages/g3rs-deny-config-checks/crates/assertions/src/` with per-rule assertion helpers and shared `common.rs`.
...
- `g3rs-deny-config-checks` owns pure typed `deny.toml` content rules
...
## Information Sources
- `packages/g3rs-deny-config-checks` current package layout and TODO notes
- existing extracted package patterns from `g3rs-clippy-config-checks` and `g3rs-cargo-config-checks`
...
- `packages/g3rs-deny-config-checks/crates/runtime/src/lib.rs` — runtime module surface after the directory-per-rule conversion
- `packages/g3rs-deny-config-checks/crates/assertions/src/lib.rs` — package-local assertion exports
- `packages/g3rs-deny-config-checks/crates/assertions/src/common.rs` — shared assertion helpers
- `packages/g3rs-deny-config-checks/TODO.md` — remaining known package issues
...
1. Audit whether `g3rs-deny-config-checks/TODO.md` still reflects the current gaps after this test migration and trim stale TODO entries if necessary.
