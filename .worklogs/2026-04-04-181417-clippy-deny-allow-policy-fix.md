...
- `g3rs-clippy/no-op-placeholder` is now effectively retired for current ban-only surfaces; decide whether to delete it or repurpose it for a real weakening surface later.
...
1. Decide whether `g3rs-clippy/no-op-placeholder` should be deleted or repurposed now that ban-only surfaces are quiet.
