...
Normalized `packages/parsers/nextest-toml-parser` to the current sibling-crate package shape and removed all package-debt errors. The only remaining signals are the 2 intentional `RS-CODE-SOURCE-04` inventory warnings on the centralized fs and parser boundary allows.
