...
use clippy_toml_parser_runtime::types::{
    ClippyToml, DisallowedField, DisallowedPath, InherentImplLintScope, MatchLintBehaviour,
    PubUnderscoreFieldsBehaviour, SourceItemOrderingWithinModuleItemGroupings,
};

pub fn assert_thresholds_empty(cfg: &ClippyToml) {
    assert_eq!(
        cfg.max_struct_bools, None,
        "max_struct_bools should be None"
    );
    assert_eq!(
...
pub fn assert_ban_lists_empty(cfg: &ClippyToml) {
    assert!(
        cfg.disallowed_methods.is_empty(),
        "disallowed_methods should be empty"
    );
    assert!(
        cfg.disallowed_types.is_empty(),
        "disallowed_types should be empty"
    );
    assert!(
        cfg.disallowed_macros.is_empty(),
        "disallowed_macros should be empty"
    );
}

#[allow(
    clippy::too_many_arguments,
    reason = "threshold tuple would be less readable at callsites"
)]
pub fn assert_thresholds(
    ...
    assert_eq!(
        cfg.max_struct_bools, max_struct_bools,
        "max_struct_bools mismatch"
    );
    assert_eq!(
...
pub fn assert_first_detailed_method(
    cfg: &ClippyToml,
    path: &str,
    reason: Option<&str>,
...
pub fn assert_first_detailed_field(
    cfg: &ClippyToml,
    path: &str,
    reason: Option<&str>,
    allow_invalid: Option<bool>,
...
pub fn assert_matches_for_let_else(cfg: &ClippyToml, expected: MatchLintBehaviour) {
    assert_eq!(
        cfg.matches_for_let_else,
        Some(expected),
        "matches_for_let_else mismatch",
    );
}

pub fn assert_inherent_impl_lint_scope(cfg: &ClippyToml, expected: InherentImplLintScope) {
    assert_eq!(
        cfg.inherent_impl_lint_scope,
        Some(expected),
        "inherent_impl_lint_scope mismatch",
    );
}

pub fn assert_pub_underscore_fields_behavior(
    cfg: &ClippyToml,
    expected: PubUnderscoreFieldsBehaviour,
) {
    assert_eq!(
        cfg.pub_underscore_fields_behavior,
        Some(expected),
        "pub_underscore_fields_behavior mismatch",
    );
}

pub fn assert_module_items_ordered_within_groupings(
    cfg: &ClippyToml,
    expected: SourceItemOrderingWithinModuleItemGroupings,
) {
    assert_eq!(
        cfg.module_items_ordered_within_groupings,
        Some(expected),
        "module_items_ordered_within_groupings mismatch",
    );
}
