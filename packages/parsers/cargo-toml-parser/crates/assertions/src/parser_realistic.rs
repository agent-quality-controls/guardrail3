...
use cargo_toml_parser_runtime::types::{CargoToml, Dependency, LintValue, Value};
...
fn assert_extra_string(manifest: &CargoToml, key: &str, expected: &str) {
    assert_eq!(
...
fn assert_simple_dep(actual: Option<&Dependency>, expected: &str, field_name: &str) {
...
fn assert_detailed_dep_version(actual: Option<&Dependency>, expected: &str, field_name: &str) {
    assert!(actual.is_some(), "{field_name} should exist");
    assert!(
...
fn assert_lint_level(actual: Option<&LintValue>, expected: &str, field_name: &str) {
    assert!(actual.is_some(), "{field_name} should exist");
    match actual {
