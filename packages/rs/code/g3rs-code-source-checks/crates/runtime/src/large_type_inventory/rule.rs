            ...
            LargeTypeItem::Struct {
                ...
                results.push(
                    G3CheckResult::new(
                        ...
                        format!(
                            "struct `{name}` has {field_count} fields (inventory threshold 15)."
                        ),
                        Some(input.rel_path.to_owned()),
                        Some(line),
                    )
                    .with_selector(format!("struct:{name}")),
                );
            }
            LargeTypeItem::Enum {
                line,
                ...
                results.push(
                    G3CheckResult::new(
                        ...
                        "large type inventory".to_owned(),
                        format!(
                            "enum `{name}` has {variant_count} items (inventory threshold 20)."
                        ),
                        Some(input.rel_path.to_owned()),
                        Some(line),
                    )
                    .with_selector(format!("enum:{name}")),
                );
            }
        }
    }
