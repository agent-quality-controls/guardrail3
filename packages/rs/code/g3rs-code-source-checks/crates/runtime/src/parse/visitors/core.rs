...
use crate::parse::helpers::{attrs_enter_test_context, path_to_string, span_line};
...
impl<'source> Visit<'source> for ForbiddenMacroVisitor {
...
impl<'source> Visit<'source> for TestExpectVisitor {
    fn visit_item_fn(&mut self, item_fn: &'source syn::ItemFn) {
        let was = self.save_and_apply_test_context(&item_fn.attrs);
        syn::visit::visit_item_fn(self, item_fn);
        self.restore_test_context(was);
    ...
    fn visit_expr_method_call(&mut self, method_call: &'source syn::ExprMethodCall) {
...
impl<'source> Visit<'source> for GenericParameterCapVisitor {
    ...
    fn visit_item_enum(&mut self, item_enum: &'source syn::ItemEnum) {
        self.push_if_over_cap(
...
impl<'source> Visit<'source> for LargeTypeVisitor {
    fn visit_item_struct(&mut self, item_struct: &'source syn::ItemStruct) {
...
impl<'source> Visit<'source> for LargeTraitVisitor {
    fn visit_item_trait(&mut self, item_trait: &'source syn::ItemTrait) {
