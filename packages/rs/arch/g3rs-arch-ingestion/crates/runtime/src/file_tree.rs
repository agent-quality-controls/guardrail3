...
/// `(module_dir_rel, module_dir)` pair returned by `module_dir_from_mod_decl`.
type ModuleDirEntry = (String, G3RsArchModuleDir);
...
/// collect module dirs fn.
fn collect_module_dirs(
    view: &CrawlView<'_>,
    crate_nodes: &[G3RsArchCrateNode],
) -> crate::run::IngestResult<Vec<G3RsArchModuleDir>> {
...
fn collect_module_dirs_from_mod_declarations(
    ...
    for rel_path in rs_file_rels {
        ...
        for item in &ast.items {
            if let Some((mod_dir, module_dir)) =
                module_dir_from_mod_decl(view, &dir, &rel_path, item)
            {
                let _ = module_dirs.insert(mod_dir, module_dir);
            }
        }
    }
...
/// Read and parse a Rust source file at `rel_path` into a `syn::File`.
fn parse_rs_file(view: &CrawlView<'_>, rel_path: &str) -> crate::run::IngestResult<syn::File> {
    let entry = view
        .entry(rel_path)
    ...
    syn::parse_file(content.strip_prefix('\u{feff}').unwrap_or(&content)).map_err(|err| {
        G3RsArchIngestionError::ParseFailed {
            path: entry.path.abs_path.clone(),
            reason: err.to_string(),
        }
    })
...
/// Build a `G3RsArchModuleDir` entry for a `mod foo;` declaration in `item`.
fn module_dir_from_mod_decl(
    view: &CrawlView<'_>,
    dir: &str,
    rel_path: &str,
    item: &syn::Item,
) -> Option<ModuleDirEntry> {
    ...
    let mod_entry = view.dir_contents(&mod_dir)?;
    let rs_file_count = mod_entry
        .files()
        .iter()
        .filter(|file| is_rs_file(file))
        .count();
    if rs_file_count == 0 {
        return None;
    }
    ...
    let has_sibling_file = view
        .dir_contents(dir)
        .is_some_and(|dir_entry| dir_entry.files().iter().any(|file| file == &sibling_file));
    Some((
        mod_dir.clone(),
        G3RsArchModuleDir {
            dir_rel: mod_dir,
            mod_decl_file: rel_path.to_owned(),
...
/// collect module dirs from directory scan fn.
