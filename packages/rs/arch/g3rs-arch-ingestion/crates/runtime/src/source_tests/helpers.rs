...
fn crawl_workspace(root: &TempDir) -> G3RsWorkspaceCrawl {
    crawl(root.path()).expect("crawl fixture workspace for arch source ingestion test")
}

pub(super) fn source_inputs(root: &TempDir) -> Vec<G3RsArchSourceChecksInput> {
    crate::source::ingest_for_source_checks(&crawl_workspace(root))
        .expect("ingest source checks from fixture workspace for arch source ingestion test")
}
