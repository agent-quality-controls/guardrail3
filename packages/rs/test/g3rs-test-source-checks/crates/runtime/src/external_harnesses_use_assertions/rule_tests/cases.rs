...
use g3rs_test_source_checks_assertions::external_harnesses_use_assertions::rule as assertions;
...
#[test]
fn inventories_external_harness_using_aliased_owned_assertions_crate() {
    let results = assertions::check(&assertions::input(
        vec![
            assertions::file(
                "assertions/src/lib.rs",
...
#[test]
fn inventories_external_harness_using_aliased_owned_assertions_through_local_alias_chain() {
    let results = assertions::check(&assertions::input(
        vec![
            assertions::file(
                "assertions/src/lib.rs",
...
#[test]
fn reports_external_harness_using_use_aliased_local_assertion_wrapper() {
    let results = assertions::check(&assertions::input(
        vec![assertions::file(
            "tests/use_alias_wrapper.rs",
            G3RsTestFileKind::ExternalHarness,
            Some("demo_assertions"),
            "fn assert_demo() { assert_eq!(1, 1); }\nuse self::assert_demo as run;\n#[test]\nfn harness() { run(); }\n",
        )],
        Some("demo_assertions"),
    ));
