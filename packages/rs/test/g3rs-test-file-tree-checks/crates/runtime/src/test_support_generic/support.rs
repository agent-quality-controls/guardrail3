...
pub(super) fn glob_imported_helper_names(
    ...
    for binding in &input.file.parsed.imports {
            ...
            .filter(|file| file.rel_path != input.file.rel_path)
        {
            if !target_paths
                .iter()
                .any(|target| target == &sibling.rel_path)
            {
                continue;
            }
...
pub(super) fn local_import_aliases(imports: &[UseBinding]) -> BTreeMap<String, Vec<String>> {
    ...
    loop {
        ...
        for binding in imports {
            ...
            if !matches!(first.as_str(), "crate" | "self" | "super") && !aliases.contains_key(first)
            {
                continue;
            }
            let Some(local_name) = binding
    ...
    match path {
        [name] => {
            ...
            local_import_aliases.contains_key(name)
                && local_call_alias_targets_local_helper(name, local_helpers, local_import_aliases)
        }
        [first, ..] if matches!(first.as_str(), "crate" | "self" | "super") => path
            .last()
