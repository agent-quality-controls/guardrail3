...
use g3rs_test_file_tree_checks_assertions::test_support_generic::rule as assertions;
...
fn reports_semantic_finding_helper_via_glob_imported_sibling_module() {
    ...
    files.push(file(
        ...
        Some("helpers"),
        None,
        "use guardrail3_domain_report::CheckResult;\nfn has_rule(results: &[CheckResult]) -> bool { results.iter().any(|result| result.id() == \"g3rs-test/test-support-generic\") }\nfn any_rule(results: &[CheckResult]) -> bool { has_rule(results) }\n",
    ));
    let results = assertions::check(&input(files, vec![active_component()]));
