...
impl<'source> Visit<'source> for TestBodyVisitor {
    fn visit_macro(&mut self, mac: &'source syn::Macro) {
        if let Some(name) = mac.path.segments.last().map(|segment| segment.ident.to_string()) {
            ...
            if matches!(name.as_str(), "assert_eq" | "assert_ne" | "debug_assert_eq" | "debug_assert_ne")
    ...
    fn visit_local(&mut self, local: &'source syn::Local) {
        ...
        if let (Some(init), Some(name)) = (local.init.as_ref(), helpers::single_pat_ident(&local.pat)) {
