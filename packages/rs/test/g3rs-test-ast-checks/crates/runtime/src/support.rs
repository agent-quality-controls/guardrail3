...
pub(crate) fn analyze_root(input: &G3RsTestAstChecksInput) -> Result<RootAnalysis, ParseFailure> {
    let mut analysis = RootAnalysis::default();

    for file in &input.files {
        let ast = parse_rust_file(&file.content).map_err(|err| ParseFailure {
            rel_path: file.rel_path.clone(),
            reason: format!("Failed to parse Rust source file for test-family AST analysis: {err}"),
        })?;
        let parsed = analyze(&ast, &file.content);
