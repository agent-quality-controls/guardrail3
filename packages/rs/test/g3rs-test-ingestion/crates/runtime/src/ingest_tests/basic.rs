...
use g3rs_test_ingestion_assertions::require_root;
...
tokio = { version = \"1\", features = [\"macros\"] }\n",
    ...
    let inputs =
        crate::run::ingest_for_config_checks_with_tool_state(&workspace_crawl, true)
            .expect("ingestion should succeed");
    assert_eq!(inputs.len(), 1, "{inputs:#?}");

    let input = require_root(&inputs, "");
    ...
    assert_eq!(input.mutation_hook_files, vec![".githooks/pre-commit".to_owned()]);
...
resolver = \"2\"\n",
    ...
    write(root.join("crates/app/src/lib.rs"), "#[test]\nfn smoke() {}\n");
    write(
        root.join("vendor/nested/Cargo.toml"),
        "[package]\nname = \"nested\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    );
    write(root.join("vendor/nested/src/lib.rs"), "#[test]\nfn hidden() {}\n");

    let workspace_crawl = crawl(root).expect("crawl should succeed");
    let inputs = crate::ingest_for_config_checks(&workspace_crawl).expect("ingestion should succeed");
