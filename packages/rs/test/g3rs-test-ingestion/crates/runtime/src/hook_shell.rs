...
pub(crate) struct ExecutableLine<'a> {
    command_text: &'a str,
}

impl<'a> ExecutableLine<'a> {
    #[must_use]
    pub(crate) fn command_text(&self) -> &'a str {
        self.command_text
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct ParsedShellScript<'a> {
    executable_lines: Vec<ExecutableLine<'a>>,
}

impl<'a> ParsedShellScript<'a> {
    #[must_use]
    pub(crate) fn executable_lines(&self) -> &[ExecutableLine<'a>] {
        self.executable_lines.as_slice()
    }
}

#[must_use]
pub(crate) fn parse_script(content: &str) -> ParsedShellScript<'_> {
    let mut executable_lines = Vec::new();
    let logical_lines = collect_logical_lines(content);
    let mut function_brace_depth = 0usize;
    let mut current_function_body = String::new();
    let mut current_function_open = false;
    ...
    for (line_no, raw) in logical_lines {
        ...
        if current_function_open {
            current_function_body.push_str(raw);
            current_function_body.push('\n');
            function_brace_depth = update_function_scope_depth(function_brace_depth, trimmed);
            if function_brace_depth == 0 {
                current_function_open = false;
        ...
        if is_function_definition_line(trimmed) {
            ...
            current_function_body = initial_function_body_fragment(raw);
            if function_brace_depth == 0 {
...
fn parse_executable_line(raw: &str, _line_no: usize) -> Option<ExecutableLine<'_>> {
    ...
    if let Some(inner_command) = assignment_command_substitution(trimmed) {
        ...
        let _ = leading_command_name(command_text)?;

        return Some(ExecutableLine { command_text });
    }
    ...
    Some(ExecutableLine { command_text })
}
