...
use mutants_toml_parser::parse as parse_mutants;
use nextest_toml_parser::parse as parse_nextest;

pub(crate) fn input() -> G3RsTestConfigChecksInput {
...
pub(crate) fn with_nextest(mut input: G3RsTestConfigChecksInput, content: &str) -> G3RsTestConfigChecksInput {
    input.nextest = Some(parse_nextest(content).expect("valid nextest fixture"));
    input
}

pub(crate) fn with_mutants(mut input: G3RsTestConfigChecksInput, content: &str) -> G3RsTestConfigChecksInput {
    input.mutants = Some(parse_mutants(content).expect("valid mutants fixture"));
