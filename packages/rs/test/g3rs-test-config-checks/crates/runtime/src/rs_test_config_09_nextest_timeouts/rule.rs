...
pub(crate) fn check(input: &G3RsTestConfigChecksInput, results: &mut Vec<G3CheckResult>) {
    let async_active =
        input.has_tests && (input.tokio_dependency_present || input.has_tokio_tests);
    if !async_active {
        results.push(
            G3CheckResult::new(
                ID.to_owned(),
    ...
    let has_slow_timeout = default_profile.and_then(|profile| profile.slow_timeout.as_ref()).is_some();
    let has_leak_timeout = default_profile.and_then(|profile| profile.leak_timeout.as_ref()).is_some();
