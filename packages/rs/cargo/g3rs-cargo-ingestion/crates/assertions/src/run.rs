...
pub fn assert_config_pipeline_stands_down_allow_rules_when_guardrail3_rs_is_invalid(
    ...
    assert!(
        !results.iter().any(|result| {
            matches!(
                result.id(),
                "RS-CARGO-CONFIG-07" | "RS-CARGO-CONFIG-11" | "RS-CARGO-CONFIG-12"
            )
        }),
        "{results:#?}"
    );
...
pub fn assert_unreadable_error(err: &g3rs_cargo_ingestion_runtime::IngestionError, expected_suffix: &str) {
...
pub fn assert_parse_failed_error(err: &g3rs_cargo_ingestion_runtime::IngestionError, expected_suffix: &str) {
