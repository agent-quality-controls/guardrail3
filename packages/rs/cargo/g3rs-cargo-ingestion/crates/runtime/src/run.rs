                        ...
                        Ok(member_rels) => {
                            for member_rel in member_rels {
                                match crate::select::select_member_manifest(crawl, &member_rel) {
                                    ...
                                    Some(member_entry) => {
                                        if let Err(error) =
                                            crate::parse::parse_raw_toml(&member_entry.path.abs_path)
                                        {
                                            match error {
                                    ...
                                    None => {
                                        missing_members.push(crate::ingest::missing_member(member_rel));
                                    }
                                }
                            }
                        }
...
fn collect_config_members(
    ...
    let member_rels = crate::select::collect_declared_member_rels(crawl, raw_cargo).map_err(|reason| {
