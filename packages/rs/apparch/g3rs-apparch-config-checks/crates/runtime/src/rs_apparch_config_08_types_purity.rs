...
const ID: &str = "RS-APPARCH-CONFIG-08";
...
pub(crate) fn check(
    ...
    if violating.is_empty() {
        results.push(
            G3CheckResult::new(
                ID.to_owned(),
                G3Severity::Info,
                format!("types crate `{}` stays pure", crate::run::display_crate(krate)),
                format!(
                    "Types crate `{}` uses only built-in pure dependencies or explicitly allowlisted externals.",
    ...
    for dep in violating {
        results.push(G3CheckResult::new(
            ...
            G3Severity::Error,
            format!(
                "types crate `{}` depends on impure external crate `{}`",
                crate::run::display_crate(krate),
                dep.dep_name
            ),
            format!(
                "Types crate `{}` may only use built-in pure externals or `allowed_deps` from guardrail3-rs.toml. `{}` was found via `{}`.",
                crate::run::display_crate(krate),
                dep.dep_name,
                dep.kind.label()
            ),
