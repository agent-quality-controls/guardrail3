...
pub(super) fn input(edges: &[(&str, &str)]) -> Vec<G3RsApparchCrateDependencyChecksInput> {
    let crates = vec![
        crate_input("db", G3RsApparchLayer::IoOutbound, "io/outbound/db/Cargo.toml"),
        crate_input("cache", G3RsApparchLayer::IoOutbound, "io/outbound/cache/Cargo.toml"),
        crate_input(
            "report-runtime",
        ...
        crate_input("service", G3RsApparchLayer::Logic, "logic/service/Cargo.toml"),
        crate_input("http", G3RsApparchLayer::IoInbound, "io/inbound/http/Cargo.toml"),
