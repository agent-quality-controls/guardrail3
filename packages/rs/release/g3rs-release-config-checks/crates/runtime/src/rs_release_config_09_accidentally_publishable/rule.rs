...
pub(crate) fn run_check(cargo_toml: &str) -> Vec<guardrail3_check_types::G3CheckResult> {
    ...
    crate::rs_release_config_09_accidentally_publishable::check(&input.crates[0], &mut results);
    results
}
...
pub(crate) fn run_check_with_workspace(
    ...
    let input = crate::lib_tests::test_support::config_input_for_crate(cargo_toml, Some(workspace_cargo_toml));
    crate::check(&input)
}
