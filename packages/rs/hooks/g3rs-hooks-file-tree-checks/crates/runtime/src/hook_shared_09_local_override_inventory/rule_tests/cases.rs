use g3rs_hooks_file_tree_checks_assertions::hook_shared_09_local_override_inventory::rule as assertions;
...
fn inventories_override_names() {
    let mut results = Vec::new();
    check(&["10-local.sh".to_owned(), "20-debug.sh".to_owned()], &mut results);
