use g3rs_hooks_file_tree_checks_assertions::hook_shared_05_pre_commit_executable::rule as assertions;
...
fn reports_non_executable_hook() {
    ...
    check(&script(".githooks/pre-commit", 2, 24, Some(false)), &mut results);
...
fn inventories_executable_hook() {
    let mut results = Vec::new();
    check(&script(".githooks/pre-commit", 2, 24, Some(true)), &mut results);
