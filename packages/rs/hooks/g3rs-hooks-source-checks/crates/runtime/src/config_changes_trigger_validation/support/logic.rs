...
use super::text::{
    expand_inline_case_block, expand_inline_if_block, is_trigger_like_line, line_contains_then,
    line_reaches_config_trigger, looks_like_case_pattern_line, mentions_config_exact,
};
use crate::guardrail_validate_staged_present::{
...
fn case_branches(block: &NumberedBlock) -> Vec<NumberedBlock> {
    if block.lines.len() == 1 {
        ...
        if expanded != block.lines[0].raw {
            return case_branches(&expanded_single_line_block(
                block.lines[0].line_no,
                &expanded,
            ));
        }
