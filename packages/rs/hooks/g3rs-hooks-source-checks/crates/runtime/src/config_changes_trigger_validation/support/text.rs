...
fn line_reaches_config_trigger_inner(
    ...
    local
        ...
        .any(|line| {
            let Some(function) =
                resolve_visible_function(local, root, &line.command_name, line_no, root_line_no)
            else {
                return false;
            };
    ...
    local
        ...
        .or_else(|| {
            if std::ptr::eq(local, root) {
                ...
                root.functions.iter().rev().find(|function| {
                    function.name == function_name && function.line_no <= root_line_no
                })
            }
        })
}
