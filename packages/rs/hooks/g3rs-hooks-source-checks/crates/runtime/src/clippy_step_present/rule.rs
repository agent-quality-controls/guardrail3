...
const ID: &str = "g3rs-hooks/clippy-step-present";

pub(crate) fn check(input: &RustHookCommandInput<'_>, results: &mut Vec<G3CheckResult>) {
    let found = any_resolved_command(input.parsed, is_cargo_clippy_command);
...
fn is_cargo_clippy_command(command: &ResolvedCommand) -> bool {
    cargo_subcommand_tail(command, "clippy").is_some_and(|args| !args_have_help_or_version(args))
}
