...
const ID: &str = "g3rs-hooks/cargo-machete-step-present";

pub(crate) fn check(input: &RustHookCommandInput<'_>, results: &mut Vec<G3CheckResult>) {
    let found = any_resolved_command(input.parsed, is_cargo_machete_command);
...
fn is_cargo_machete_command(command: &ResolvedCommand) -> bool {
    match command.command_name() {
        "cargo" => cargo_subcommand_tail(command, "machete")
            .is_some_and(|args| !args_have_help_or_version(args)),
        "cargo-machete" => !args_have_help_or_version(command.args()),
        _ => false,
    }
}
