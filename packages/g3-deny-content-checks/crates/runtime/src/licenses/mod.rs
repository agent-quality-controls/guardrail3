...
pub(crate) fn check(deny_rel_path: &str, deny: &DenyToml, results: &mut Vec<G3CheckResult>) {
    rs_deny_14_license_allow_baseline::check(deny_rel_path, deny, results);
    rs_deny_15_confidence_threshold::check(deny_rel_path, deny, results);
    rs_deny_16_copyleft_allowlist::check(deny_rel_path, deny, results);
}
