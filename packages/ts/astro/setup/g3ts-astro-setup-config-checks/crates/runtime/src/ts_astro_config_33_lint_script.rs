...
const ID: &str = "TS-ASTRO-SETUP-CONFIG-33";
...
pub(crate) fn check(
    ...
    let message = match rel_path {
        ...
        None => "The Astro lint execution contract could not be checked because `package.json` was not available. Restore the app package manifest and add a fail-closed `lint` script that invokes `eslint`.".to_owned(),
    };
