...
use g3ts_astro_types::{
    G3TsAstroAppRootInput, G3TsAstroCrossRootSideLoaderInput, G3TsAstroFileTreeChecksInput,
...
#[test]
fn cross_root_side_loaders_are_forbidden() {
    let mut input = golden_build_collections_input();
    input.cross_root_side_loaders
        .push(G3TsAstroCrossRootSideLoaderInput {
            loader_rel_path: "src/lib/kit-taxonomy.ts".to_owned(),
            target_rel_path: "../packages/content-spec/taxonomy.json".to_owned(),
        });

    let results = crate::run::check(&input);
    assertions::assert_has_error(&results, "TS-ASTRO-FILETREE-05");
}
