...
pub fn checks_options_are_fail_closed(value: &G3TsAstroStaticValue) -> bool {
...
pub fn checks_options_have_structured_data_custom_check(value: &G3TsAstroStaticValue) -> bool {
    let Some(properties) = object_properties(value) else {
        return false;
    };
    if object_has_duplicate_keys(properties) {
