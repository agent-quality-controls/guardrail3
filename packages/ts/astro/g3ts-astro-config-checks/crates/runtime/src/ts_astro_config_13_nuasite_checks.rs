...
const ID: &str = "TS-ASTRO-CONFIG-13";
...
pub(crate) fn check(input: &G3TsAstroConfigChecksInput, results: &mut Vec<G3CheckResult>) {
    for contract in &input.integration_contracts {
        ...
        let has_build_script = crate::support::package_safely_runs_astro_build(contract);
        let has_static_output = crate::support::astro_config_is_static(contract);
        let has_checks = match &contract.astro_config {
            G3TsAstroConfigSurfaceState::Parsed { snapshot } => {
                crate::support::astro_config_has_nuasite_checks_with_required_options(snapshot)
            }
