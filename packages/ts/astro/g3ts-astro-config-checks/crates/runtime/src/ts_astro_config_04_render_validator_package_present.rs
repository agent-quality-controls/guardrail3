...
pub(crate) fn check(input: &G3TsAstroConfigChecksInput, results: &mut Vec<G3CheckResult>) {
    for contract in &input.integration_contracts {
        if !contract.requires_render_validator {
            continue;
        }
        ...
        let message = match rel_path {
            Some(rel_path) => format!(
                "`{rel_path}` does not include the required Astro render validator package `{DEPENDENCY_NAME}`."
            ),
            None => format!(
                "Could not verify the required Astro render validator package `{DEPENDENCY_NAME}` because no package manifest was available."
            ),
        };
