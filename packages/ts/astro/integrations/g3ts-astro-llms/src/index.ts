...
export type G3TsAstroLlmsConfig = z.input<typeof llmsConfigSchema>;
...
export default function g3tsLlms(config: G3TsAstroLlmsConfig): AstroIntegration {
  const parsedConfig = parseConfig(config);

  return {
    name: "g3ts-astro-llms",
    hooks: {
...
export function generateLlmsTxt(config: G3TsAstroLlmsConfig): string {
...
function parseConfig(config: G3TsAstroLlmsConfig): ParsedLlmsConfig {
  ...
  throw new Error(`Invalid g3ts-astro-llms config: ${message}`);
}
...
function normalizeHref(site: string, href: string): string {
  ...
  if (url.protocol !== "https:") {
    throw new Error("Invalid g3ts-astro-llms config: href must resolve to HTTPS");
  }
  if (url.host !== canonicalSite.host) {
    throw new Error("Invalid g3ts-astro-llms config: href must stay on the configured site host");
  }
  url.hash = "";
  return url.toString();
}
