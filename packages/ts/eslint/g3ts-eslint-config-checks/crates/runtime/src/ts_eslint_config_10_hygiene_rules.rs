...
pub(crate) fn check(input: &G3TsEslintConfigChecksInput, results: &mut Vec<G3CheckResult>) {
    ...
    let missing = missing_error_rules(input, EslintProbeKind::TsSource, HYGIENE_RULES);
    if missing.is_empty() {
        results.push(info(
            ID,
    ...
    results.push(error(
        ...
        format!(
            "The TS source probe must enforce these hygiene rules at error severity: {}.",
