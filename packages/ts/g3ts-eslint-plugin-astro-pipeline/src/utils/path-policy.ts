...
export function resolvePathLike(
  ...
  if (rawPath.startsWith("@/") || rawPath.startsWith("~/")) {
    return normalizePathFromCwd(path.resolve(cwd, "src", rawPath.slice(2)), cwd);
  }

  if (rawPath === "@content" || rawPath.startsWith("@content/")) {
    const contentPath = rawPath === "@content" ? "" : rawPath.slice("@content/".length);

    return normalizePathFromCwd(path.resolve(cwd, "src", "content", contentPath), cwd);
  }
