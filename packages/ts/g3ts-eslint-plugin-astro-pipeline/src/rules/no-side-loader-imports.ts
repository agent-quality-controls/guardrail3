...
function findForbiddenSideLoader(
  ...
  if (
    ...
    if (!programImportsAstroContent(program, scopeManager)) {
      return [];
    }
...
function programImportsAstroContent(
  program: TSESTree.Program,
  scopeManager: ReturnType<typeof collectImportClosure>[number]["scopeManager"]
): boolean {
  ...
  return (
    hasDynamicAstroContentImport(program, importAliases, scopeManager) ||
    hasRequireAstroContentImport(program, imports, requireAliases, importAliases, scopeManager)
  );
}
...
function hasDynamicAstroContentImport(
  ...
  findNodes(program, (node) => {
    ...
    if (
      resolveStaticStringExpression(node.source, constants, new Set(), scopeManager) ===
      "astro:content"
    ) {
      found = true;
    }
...
function hasRequireAstroContentImport(
  ...
  findNodes(program, (node) => {
    ...
    if (getRequireSource(node, constants, scopeManager) === "astro:content") {
      found = true;
    }
  });
...
function getRequireSource(
  ...
  return resolveStaticStringExpression(firstArg, constants, new Set(), scopeManager);
}
