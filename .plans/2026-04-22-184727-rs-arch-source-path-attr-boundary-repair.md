...
Repair the remaining `rs/arch` source-boundary defect by moving `#[path]` analysis for `g3rs-arch/no-path-attr` into ingestion. `g3rs-arch-source-checks` should no longer receive raw `source_files` just to let one rule parse them locally.
...
4. Rewrite `g3rs-arch/no-path-attr` to consume prebound `path_attr_sites`.
...
- Drop `source_files` from the source lane entirely.
  - Why: after `g3rs-arch/no-path-attr` moves to ingestion, no source rule needs the raw file bag.
