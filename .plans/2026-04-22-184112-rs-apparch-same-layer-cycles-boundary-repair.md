...
Repair the remaining `rs/apparch` config boundary defect in `RS-APPARCH-CONFIG-06` so the rule no longer reconstructs a crate index from edge payloads. Ingestion should bind the same-layer node set explicitly and the config check should consume that prebound input directly.
...
4. Simplify `RS-APPARCH-CONFIG-06` to consume the prebound nodes directly.
