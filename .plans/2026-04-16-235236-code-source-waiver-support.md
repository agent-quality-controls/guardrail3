Goal
- Make `guardrail3-rs.toml` waivers actually reach the `code` source family so package-local waivers can stand down `g3rs-code/ast-19-large-type-inventory` without changing the rule meaning.

Approach
- Read the existing code ingestion and code source rule tests to find the smallest typed place to thread waivers.
- Add tests first that prove two cases: a matching `g3rs-code/ast-19-large-type-inventory` waiver suppresses the large-type warning, and a non-matching waiver does not.
- Add a minimal code-family waiver type and carry parsed `guardrail3-rs.toml` waivers into `G3RsCodeSourceChecksInput`.
- Update `CodeSourceRuleInput` and `g3rs-code/ast-19-large-type-inventory` to honor only an exact matching waiver on the file and selector.
...
- Do not weaken `g3rs-code/ast-19-large-type-inventory` globally; only add the already-established package-local waiver mechanism.
