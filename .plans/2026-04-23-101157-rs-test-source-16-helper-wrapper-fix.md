...
- Fix the `RS-TEST-SOURCE-16` helper-wrapper gap only if proven with red tests.
...
- Run the targeted `RS-TEST-SOURCE-16` package tests first and confirm they fail.
- Fix the bug at the owning `RS-TEST-SOURCE-16` rule boundary unless the red tests prove ingestion is missing the required call/import facts.
...
- Keep the fix local to `RS-TEST-SOURCE-16` or shared `rs/test` source-check support if the same helper-resolution logic needs to be shared cleanly.
