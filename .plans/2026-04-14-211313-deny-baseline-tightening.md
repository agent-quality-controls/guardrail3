...
# Approach
1. Add or update direct rule tests for `RS-DENY-CONFIG-02` so the golden baseline requires `yanked = "deny"` and wrong-value coverage rejects `warn`.
2. Add or update direct rule tests for `RS-DENY-CONFIG-27` so the managed `regex` ban wrapper set is empty and any remaining wrapper carveout errors.
