...
Move the TS ESLint config-content rule files back to the runtime crate root and waive `RS-ARCH-FILETREE-07` for the runtime crate, matching the established rule-package pattern used elsewhere in the repo.
...
- Add a `RS-ARCH-FILETREE-07` waiver for `crates/runtime/Cargo.toml` in `packages/ts/eslint/g3ts-eslint-config-checks/guardrail3-rs.toml`.
- Run:
  - `cargo test`
  - `cargo fmt --check`
...
- Follow the existing waiver pattern instead of hiding the rule in an arbitrary nested module folder.
  - Other mature rule packages already waive `RS-ARCH-FILETREE-07` for intentionally one-rule-per-file runtime crates.
