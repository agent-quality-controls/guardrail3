...
| `RS-DENY-09` | `rs_deny_09_ban_baseline_complete.rs` | `rs_deny_09_ban_baseline_complete_tests/` | generated service/library baseline tests, missing canonical bans, fail-closed section checks, managed-wrapper integrity errors, and exact generator-vs-checker parity for both service and library ban sets | tighten false-positive controls for project-specific extras; broaden mixed-root/profile attacks where local deny roots replace ancestor policy |
...
| `RS-DENY-25` | `rs_deny_25_allow_override_channel.rs` | `rs_deny_25_allow_override_channel_tests/` | canonical generated no-allow-list non-hit path, non-empty allow-list error, one override error per allowed banned crate, explicit non-overlapping allow-list coverage, and local-root ownership replacement for allow-list drift | broader mixed-profile attacks beyond the current local service-root case |
