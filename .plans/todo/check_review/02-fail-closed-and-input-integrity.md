...
- `RS-CARGO-05` implementation is broader than the plan text:
...
- concrete under-implementation notes still matter:
  - `RS-FMT-04` currently depends only on repo-root toolchain facts
  - `RS-FMT-06` currently compares against repo-root Cargo metadata only
...
- `RS-TOOLCHAIN-03` still lacks profile context for the library-specific behavior the plan describes.
