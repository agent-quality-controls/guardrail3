...
| `toolchain_check.rs` | All sub-checks share ID "R25" — JSON filtering ambiguous | Split into distinct sub-IDs during RS-TOOLCHAIN-02 migration. |
...
| `workspace_metadata.rs` wiring | RS-CARGO-05 called from config_files.rs, not cargo_lints.rs | Move call into cargo_lints module |
...
| `deny_inventory.rs` RS-DENY-12 | Malformed skip entries silently report "unknown" | Warn on entries missing both `crate` and `name` fields |
| `deny_inventory.rs` RS-DENY-12 | `reason` field present but not a string treated as empty | Warn on wrong type |
| `deny_inventory.rs` RS-DENY-13 | Missing `.as_inventory()` on advisory ignore Info entries | Add for `--inventory` consistency |
