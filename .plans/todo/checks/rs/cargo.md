...
So:
- `g3rs-cargo/workspace-lints` checks completeness in `[workspace.lints.*]` for workspace roots and `[lints.*]` for standalone package roots
- `g3rs-cargo/lint-levels` checks level correctness in `[workspace.lints.*]` for workspace roots and `[lints.*]` for standalone package roots
...
| g3rs-cargo/priority-order | R55 | Error | Policy-root edition policy. Missing edition or edition below 2021 is Error. | Implemented |
...
| g3rs-cargo/resolver | Warn | Lint group priority ordering. Specific lints with `priority < 0` get overridden by groups at `priority = -1`. Verify all specific deny lints have priority >= 0 (or no priority, default 0) so they take precedence over groups. Flag any specific lint with `priority < 0`. | Implemented |
| g3rs-cargo/disallowed-macros-deny | Error | Workspace resolver enforcement. Every workspace root must set explicit `resolver = "2"` or `"3"`. Standalone packages are out of scope for this rule. | Implemented |
...
| New ID | Severity | What | Status |
|--------|----------|------|--------|
| g3rs-cargo/approved-allow-inventory | Error | Canonical lint baseline must include `clippy::disallowed_macros = "deny"` so `RS-CLIPPY-20` is not toothless. | Implemented |
...
- `RS-CARGO-15` and `g3rs-toolchain/msrv-consistency` intentionally overlap around MSRV/toolchain compatibility
...
|---------|-------------|
| `[profile.dev/test]` settings | Subjective. Release profile already covered by g3rs-cargo/priority-order. |
...
| Lint name typos | `warnings = "deny"` (g3rs-cargo/workspace-lints) + cargo clippy catches unknown lint names at compile time. |
| Keep `rust-version` folded into g3rs-cargo/priority-order | Rejected. Edition policy and MSRV policy are now split across `g3rs-cargo/priority-order` and `RS-CARGO-15`. |
