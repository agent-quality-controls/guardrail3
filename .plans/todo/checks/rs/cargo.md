...
So:
- `RS-CARGO-CONFIG-01` checks completeness in `[workspace.lints.*]` for workspace roots and `[lints.*]` for standalone package roots
- `RS-CARGO-CONFIG-02` checks level correctness in `[workspace.lints.*]` for workspace roots and `[lints.*]` for standalone package roots
...
| RS-CARGO-CONFIG-04 | R55 | Error | Policy-root edition policy. Missing edition or edition below 2021 is Error. | Implemented |
...
| RS-CARGO-CONFIG-05 | Warn | Lint group priority ordering. Specific lints with `priority < 0` get overridden by groups at `priority = -1`. Verify all specific deny lints have priority >= 0 (or no priority, default 0) so they take precedence over groups. Flag any specific lint with `priority < 0`. | Implemented |
| RS-CARGO-CONFIG-06 | Error | Workspace resolver enforcement. Every workspace root must set explicit `resolver = "2"` or `"3"`. Standalone packages are out of scope for this rule. | Implemented |
...
| New ID | Severity | What | Status |
|--------|----------|------|--------|
| RS-CARGO-CONFIG-07 | Error | Canonical lint baseline must include `clippy::disallowed_macros = "deny"` so `RS-CLIPPY-20` is not toothless. | Implemented |
...
- `RS-CARGO-15` and `RS-TOOLCHAIN-CONFIG-02` intentionally overlap around MSRV/toolchain compatibility
...
|---------|-------------|
| `[profile.dev/test]` settings | Subjective. Release profile already covered by RS-CARGO-CONFIG-04. |
...
| Lint name typos | `warnings = "deny"` (RS-CARGO-CONFIG-01) + cargo clippy catches unknown lint names at compile time. |
| Keep `rust-version` folded into RS-CARGO-CONFIG-04 | Rejected. Edition policy and MSRV policy are now split across `RS-CARGO-CONFIG-04` and `RS-CARGO-15`. |
