...
| RS-CARGO-05 | R55 | Error | Policy-root edition policy. Missing edition or edition below 2021 is Error. | Planned as split from current mixed metadata rule |
| RS-CARGO-14 | — | Error | Cargo-family input failures: malformed owned-root `Cargo.toml`, malformed member `Cargo.toml` for workspace-member rules, or malformed root-local `guardrail3.toml` when profile-sensitive expectations are needed. | Planned |
| RS-CARGO-15 | — | Error/Info | Policy-root `rust-version` / MSRV policy. Library profile missing `rust-version` is Error because MSRV is a compatibility contract; service profile may inventory only. | Planned as split from current mixed metadata rule |
...
| RS-CARGO-08 | Error | Workspace resolver enforcement. Every workspace root must set explicit `resolver = "2"` or `"3"`. Standalone packages are out of scope for this rule. | Implemented, but semantics still softer than desired |
...
| New ID | Severity | What | Status |
|--------|----------|------|--------|
| RS-CARGO-11 | Error | Canonical lint baseline must include `clippy::disallowed_macros = "deny"` so `RS-CLIPPY-20` is not toothless. | Planned |
| RS-CARGO-12 | Error | Any explicit `allow` outside the approved allowlist should be surfaced; today `RS-CARGO-03` inventories only the known approved set. | Planned |
