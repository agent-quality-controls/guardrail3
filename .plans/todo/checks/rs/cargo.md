...
| Location | Bug | Fix |
|----------|-----|-----|
| `cargo_lints.rs` line 313 | `crate_cargo.exists()` bypasses FileSystem trait | Change to `fs.metadata(&crate_cargo).is_none()`. Also emit Warn when a declared member has no Cargo.toml (silent skip currently). |
