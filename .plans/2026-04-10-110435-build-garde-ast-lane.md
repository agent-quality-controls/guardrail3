...
Build the real `g3rs-garde` AST lane end to end so `crawl -> ingest_for_source_checks -> g3rs-garde-source-checks::check` works, and preserve fail-closed `RS-GARDE-10` behavior for owned unreadable or malformed inputs.

## Approach

1. Prove the current extracted lane gap with tests.
   - Add ingestion tests that expect real `ingest_for_source_checks(...)` output.
   - Add AST package tests that prove unreadable and malformed source currently fail open.

2. Fix the AST package boundary first.
   - Extend `g3rs-garde-source-checks` so it can emit `RS-GARDE-10` for:
...
3. Implement root-scoped `g3rs-garde-ingestion::ingest_for_source_checks(...)`.
   - Select owned Rust source files from the crawl.
   - Skip fixture paths.
   - Require root `guardrail3.toml`.
   - Build one `G3RsGardeSourceChecksInput` for the crawled root.
