        ...
        match base_name {
            "todo" | "unimplemented" => {
                results.push(CheckResult {
                    ...
                    message: format!("`{name}!()` macro found: {message}. This panics at runtime, indicating incomplete implementation. Replace with actual logic or return an error."),
            ...
            "unreachable" if !is_test_file => {
                results.push(CheckResult {
                    ...
                    message: format!("`unreachable!()` macro found: {message}. This panics if reached, asserting a code path is impossible. Verify the assertion is correct."),
                    file: Some(path.display().to_string()),
                    line: Some(line),
                    inventory: false,
                });
...
pub fn check_unwrap_expect(path: &Path, content: &str, results: &mut Vec<CheckResult>) {
    ...
    for (line, method) in ast_helpers::find_unwrap_expect(&file) {
        ...
        results.push(CheckResult {
            ...
            message: format!("`.{method}()` found: {message}. This panics on None/Err instead of handling the error gracefully. Use `?`, `if let`, `match`, or `.unwrap_or()` instead."),
            file: Some(path.display().to_string()),
            line: Some(line),
            inventory: false,
        });
...
pub fn check_file_exists_at_root(
    ...
    if file_path.exists() {
        results.push(CheckResult {
            ...
            message: format!("{filename} found at project root. Required project file present, no action needed."),
