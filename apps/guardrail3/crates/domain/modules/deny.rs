...
pub fn build_deny_toml_with_entries(
    ...
    let _ = writeln!(
        out,
        "# deny.toml -- GENERATED by guardrail3 (profile: {profile})"
    );
    ...
    // Skip entries
    out.push_str("skip = [\n");
    let deduped_skip = crate::adapters::inbound::cli::generate::deduplicated_override(&out, extra_skip);
    ...
    let deduped_bans = crate::adapters::inbound::cli::generate::deduplicated_override(&out, extra_bans);
    ...
    let deduped_feature_bans =
        crate::adapters::inbound::cli::generate::deduplicated_override(&out, extra_feature_bans);
