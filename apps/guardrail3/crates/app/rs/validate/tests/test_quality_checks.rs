...
pub fn check_test_coverage_inventory(
    ...
    "R-TEST-05".to_owned(),
    Severity::Info,
    "Test coverage inventory".to_owned(),
    format!(
...
pub fn check_integration_tests(
    ...
    if has_rs_files_in_dir(fs, &tests_dir) {
        results.push(
            CheckResult::from_parts(
                "R-TEST-06".to_owned(),
                Severity::Info,
                "Integration tests exist".to_owned(),
                "tests/ directory with .rs files found".to_owned(),
                Some(tests_dir.display().to_string()),
                None,
                false,
            )
            .as_inventory(),
    ...
    "R-TEST-06".to_owned(),
    Severity::Info,
    "No integration tests".to_owned(),
    "No tests/ directory with .rs files found".to_owned(),
    None,
    None,
    false,
    ));,
...
pub fn check_ignore_without_reason(
    ...
    "R-TEST-07".to_owned(),
    Severity::Warn,
    "#[ignore] without reason".to_owned(),
    "`#[ignore]` attribute without a documented reason. Ignored tests without reasons become permanently forgotten — no one knows if the issue was fixed or if the test is still relevant. Add `// reason: <why>` on the same line. Example: `#[ignore] // reason: requires external service`"
                    .to_owned(),
    Some(entry.path().display().to_string()),
    Some(*line_num),
    false,
    ...
    "R-TEST-07".to_owned(),
    Severity::Info,
    "All #[ignore] have reasons".to_owned(),
    "No bare `#[ignore]` attributes found — all ignored tests have documented reasons. This ensures ignored tests are tracked and revisited. No action needed.".to_owned(),
    None,
    None,
    false,
        }.as_inventory());
...
pub fn check_mutation_hook(
                ...
                if content.contains("mutant") || content.contains("cargo-mutants") {
                    results.push(
                        CheckResult::from_parts(
                            "R-TEST-08".to_owned(),
                            Severity::Info,
                            "Mutation test hook configured".to_owned(),
                            format!("Mutation testing hook found in {}", path.display()),
            ...
            || content.contains("stryker")
        {
            results.push(
                CheckResult::from_parts(
                    ...
                    Severity::Info,
                    "Mutation test hook configured".to_owned(),
                    "Mutation testing found in pre-commit hook".to_owned(),
                    Some(pre_commit.display().to_string()),
                    None,
                    false,
                )
                .as_inventory(),
    ...
    "R-TEST-08".to_owned(),
    Severity::Warn,
    "No mutation test hook".to_owned(),
    "No mutation testing hook found in `.claude/` or `.git/hooks/pre-commit`. Mutation testing (cargo-mutants) injects bugs to verify tests catch them — without a hook, it won't run automatically. Add a `.claude/` hook config or a pre-commit hook step that runs `cargo mutants`.".to_owned(),
    None,
    None,
    false,
    ));,
