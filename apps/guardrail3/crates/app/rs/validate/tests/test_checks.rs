...
pub fn check_mutants_toml(workspace_root: &Path, results: &mut Vec<CheckResult>) {
    ...
    if mutants_path.exists() {
        results.push(
            CheckResult::from_parts(
                "R-TEST-02".to_owned(),
                Severity::Info,
                ".cargo/mutants.toml exists".to_owned(),
                "Mutation testing configuration found".to_owned(),
                Some(mutants_path.display().to_string()),
                None,
                false,
            )
            .as_inventory(),
    ...
    "R-TEST-02".to_owned(),
    Severity::Warn,
    ".cargo/mutants.toml missing".to_owned(),
    "Mutation testing config `.cargo/mutants.toml` not found. Mutation testing (cargo-mutants) injects bugs into code to verify tests catch them — without config, it uses defaults that may be too slow or skip important targets. Create `.cargo/mutants.toml` with `timeout_multiplier = 2.0`.".to_owned(),
    Some(mutants_path.display().to_string()),
    None,
    false,
        ));
...
fn check_mutants_profile(
    ...
    "R-TEST-03".to_owned(),
    Severity::Info,
    "Cargo.toml not found".to_owned(),
    "Cannot check for [profile.mutants] without Cargo.toml".to_owned(),
    None,
    None,
    false,
        ));
    ...
    if has_mutants_profile(&content) {
        results.push(
            CheckResult::from_parts(
                ...
                "Optimized build profile for mutation testing found in Cargo.toml"
                    .to_owned(),
    ...
    "No [profile.mutants] section in Cargo.toml. This custom build profile optimizes mutation testing speed (e.g., opt-level=0, no LTO). Without it, cargo-mutants uses the `dev` profile which may be slower. Add `[profile.mutants]` with `inherits = \"dev\"` and `opt-level = 0`.".to_owned(),
    Some(cargo_path.display().to_string()),
    None,
    false,
        ));
...
fn check_tests_exist(fs: &dyn FileSystem, workspace_root: &Path, results: &mut Vec<CheckResult>) {
    ...
    "R-TEST-04".to_owned(),
    Severity::Info,
    "Tests exist".to_owned(),
    "At least one `#[test]` or `#[tokio::test]` function found in the workspace. Test presence confirmed, no action needed.".to_owned(),
    None,
    None,
    false,
        }.as_inventory());
    ...
    "No `#[test]` or `#[tokio::test]` functions found anywhere in the workspace. Without tests, bugs go undetected and refactoring is unsafe. Add test functions in a `tests/` directory.".to_owned(),
    None,
    None,
    false,
        ));
...
pub fn check_no_tests_in_src(
    ...
    "R-TEST-09".to_owned(),
    Severity::Error,
    "Inline test code in production source".to_owned(),
    format!(
