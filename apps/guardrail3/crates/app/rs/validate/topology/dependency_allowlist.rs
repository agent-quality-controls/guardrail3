...
pub fn check_dependency_allowlist(
    ...
    "R-DEPS-01".to_owned(),
    Severity::Error,
    format!("Cargo.toml unreadable for {crate_name}"),
    format!(
    ...
    format!("Invalid TOML in Cargo.toml for crate `{crate_name}`: {e}"),
    Some(cargo_path.display().to_string()),
    None,
    false,
            });
...
pub fn check_library_has_allowlist(
    ...
    "R-DEPS-02".to_owned(),
    Severity::Warn,
    format!("Library crate `{crate_name}` has no dependency allowlist"),
    format!(
