...
pub fn check_file_length(
    ...
    format!("{effective_lines} effective lines (max 500). Long files are hard to understand, review, and maintain. Extract helper functions into submodules or split the file by responsibility."),
...
pub fn check_use_count(path: &Path, content: &str, is_test: bool, results: &mut Vec<CheckResult>) {
    ...
    format!("{use_count} `use` import statements (max 20). Too many imports indicate the file has too many responsibilities or dependencies. Consolidate with `use crate::{{a, b, c}};` or split the file into smaller modules."),
    Some(path.display().to_string()),
    None,
    false,
        ));
...
pub fn check_unsafe_code_forbid(
    ...
    "unsafe_code is set to \"forbid\" in workspace lints — cannot be overridden per-crate with #[allow(unsafe_code)]. This is the strongest safety setting. No action needed.".to_owned(),
