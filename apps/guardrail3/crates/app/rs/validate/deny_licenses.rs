    ...
    match licenses.get("allow").and_then(|a| a.as_array()) {
        Some(arr) if !arr.is_empty() => {
            ...
            results.push(CheckResult {
                ...
                message: format!("deny.toml [licenses].allow permits: {}. Only dependencies with these licenses are allowed. No action needed.", license_list.join(", ")),
                file: Some(file_path.display().to_string()),
                line: None,
                inventory: false,
            }.as_inventory());
        ...
        _ => {
            results.push(CheckResult {
                ...
                message: "[licenses].allow is missing or empty in deny.toml. Without a license allow list, cargo-deny cannot enforce license compliance — dependencies with incompatible licenses (e.g., GPL) could slip in. Add `allow = [\"MIT\", \"Apache-2.0\", \"BSD-3-Clause\"]` to [licenses].".to_owned(),
                file: Some(file_path.display().to_string()),
                line: None,
                inventory: false,
            });
