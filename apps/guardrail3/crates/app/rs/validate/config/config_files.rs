...
pub fn check(
    ...
    "R1".to_owned(),
    Severity::Info,
    "clippy.toml exists".to_owned(),
    "clippy.toml found at workspace root. This file configures clippy lint thresholds and method/type bans. No action needed.".to_owned(),
...
pub fn check_per_crate_clippy(
        ...
        if let Some(crate_clippy) = crate_clippy {
            results.push(
                CheckResult::from_parts(
                    "R2".to_owned(),
                    Severity::Info,
                    "Per-crate clippy.toml".to_owned(),
                    format!("Found for {member}"),
                    Some(crate_clippy.display().to_string()),
                    None,
                    false,
                )
                .as_inventory(),
    ...
    "R2".to_owned(),
    Severity::Warn,
    "Per-crate clippy.toml missing".to_owned(),
    format!("No clippy.toml for {member}"),
    Some(crate_dir.display().to_string()),
    None,
    false,
            });
        }
...
fn check_per_crate_clippy_content(
    ...
    "R2".to_owned(),
    Severity::Error,
    format!("{member}: clippy.toml unreadable"),
    format!("Failed to read per-crate clippy.toml for {member}"),
        ...
        if found_global_bans.is_empty() {
            results.push(
                CheckResult::from_parts(
                    "R2".to_owned(),
                    Severity::Info,
                    format!("{member}: no global-state type bans"),
                    "No LazyLock/OnceLock/once_cell bans in per-crate clippy.toml"
                        .to_owned(),
                    Some(path.display().to_string()),
                    None,
                    false,
            ...
            results.push(
                CheckResult::from_parts(
                    "R2".to_owned(),
                    Severity::Info,
                    format!("{member}: global-state type bans present"),
                    format!("Bans: {}", found_global_bans.join(", ")),
                    Some(path.display().to_string()),
                    None,
                    false,
                )
...
fn check_clippy_int_threshold(
    ...
    format!("Expected {expected_val}, got {v}"),
    Some(path.display().to_string()),
    None,
    false,
            });
