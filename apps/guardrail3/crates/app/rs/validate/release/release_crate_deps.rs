...
pub fn check_publish_dry_run(
    ...
    "R-PUB-09".to_owned(),
    Severity::Error,
    format!("{}: publish dry-run error", krate.name),
    "Could not run cargo publish --dry-run".to_owned(),
    Some(krate.cargo_toml_path.display().to_string()),
    None,
    false,
            });
...
pub fn check_path_deps(
    ...
    if bad_deps.is_empty() {
        results.push(
            CheckResult::from_parts(
                "R-PUB-10".to_owned(),
                Severity::Info,
                format!("{}: path deps OK", krate.name),
                "No path dependencies to non-publishable crates".to_owned(),
    ...
    "R-PUB-10".to_owned(),
    Severity::Error,
    format!("{}: path dep to non-publishable crate", krate.name),
    format!("Depends on {bad} which is not publishable"),
    file.map(std::borrow::ToOwned::to_owned),
    None,
    false,
            ));
...
pub fn check_version_consistency(
    ...
    "R-PUB-11".to_owned(),
    Severity::Error,
    format!("{}: version mismatch with {dep_name}", krate.name),
    format!(
