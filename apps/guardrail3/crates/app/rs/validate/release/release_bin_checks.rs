...
pub fn check_binary_release_workflow(
    ...
    if has_release_build {
        results.push(
            CheckResult::from_parts(
                "R-BIN-01".to_owned(),
                Severity::Info,
                format!("{name}: binary release workflow found"),
                "Workflow builds --release with action-gh-release".to_owned(),
                None,
                None,
                false,
            )
    ...
    "R-BIN-01".to_owned(),
    Severity::Info,
    format!("{name}: no binary release workflow"),
    "No workflow building --release with action-gh-release".to_owned(),
    None,
    None,
    false,
        ));
    ...
    if has_linux {
        results.push(
            CheckResult::from_parts(
                "R-BIN-02".to_owned(),
                Severity::Info,
                format!("{name}: linux target in workflow"),
                "Workflow references linux/x86_64/amd64/ubuntu".to_owned(),
                None,
                None,
                false,
            )
        ...
        results.push(
            CheckResult::from_parts(
                "R-BIN-02".to_owned(),
                Severity::Info,
                format!("{name}: no linux target in workflow"),
                "No workflow references linux/x86_64/amd64/ubuntu".to_owned(),
                None,
                None,
                false,
            )
...
pub fn check_binstall_metadata(krate: &CrateInfo, results: &mut Vec<CheckResult>) {
    ...
    "[package.metadata.binstall] found in Cargo.toml. cargo-binstall can download pre-built binaries instead of compiling from source. No action needed.".to_owned(),
    Some(krate.cargo_toml_path.display().to_string()),
    None,
    false,
        }.as_inventory());
    } else {
