...
pub fn check_license_file(workspace_root: &Path, results: &mut Vec<CheckResult>) {
    ...
    if found {
        results.push(CheckResult {
            ...
            message: "LICENSE file found at repo root. Required for open-source publishing and legal compliance. No action needed.".to_owned(),
        ...
        results.push(CheckResult {
            ...
            message: "No LICENSE file at repo root. Without a license, the code is legally unusable by others. Create `LICENSE` (or `LICENSE-MIT`, `LICENSE-APACHE`) with your license text."
                .to_owned(),
            file: Some(workspace_root.display().to_string()),
            line: None,
            inventory: false,
...
pub fn check_workflow_contains(
    ...
    if found {
        ...
        results.push(CheckResult {
            ...
            message: format!("{missing_msg}. This CI check is recommended for automated release quality enforcement. Add it to a `.github/workflows/*.yml` file."),
            file: None,
            line: None,
            inventory: false,
        });
...
pub fn check_tool_installed(
    ...
    if tc.is_installed(tool_name) {
        results.push(CheckResult {
            ...
            message: format!("`{tool_name}` found on PATH. Required tool for release/quality checks is available. No action needed."),
            file: None,
            line: None,
            inventory: false,
        }.as_inventory());
        ...
        results.push(CheckResult {
            ...
            message: format!("`{tool_name}` not found on PATH. This tool is needed for release quality checks. Install with: `{install_cmd}`"),
            file: None,
            line: None,
            inventory: false,
        });
