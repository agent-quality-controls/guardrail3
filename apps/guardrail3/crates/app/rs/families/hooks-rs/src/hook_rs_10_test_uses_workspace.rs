...
fn line_contains_workspace_test(
    ...
    if segments.is_empty() {
        return segment_contains_workspace_test(raw, root, visiting);
    }
    ...
    for segment in segments {
        ...
        if reachable && segment.operator_after != Some("&") && segment.operator_after != Some("|") {
            if segment_contains_workspace_test(&segment.text, root, visiting) {
                return true;
            }
            for substitution in extract_command_substitutions(&segment.text) {
                if line_contains_workspace_test(&substitution, root, visiting) {
                    return true;
                }
            }
        }
...
fn segment_contains_workspace_test(
    segment: &str,
    root: &ParsedShellScript<'_>,
    visiting: &mut Vec<String>,
) -> bool {
    ...
    match normalize_command_token(first) {
        "env" => env_wrapper_contains_workspace_test(parts, root, visiting),
        "sh" | "bash" => shell_wrapper_contains_workspace_test(parts, root, visiting),
        "command" => command_wrapper_contains_workspace_test(parts, root, visiting),
        "exec" => exec_wrapper_contains_workspace_test(parts, root, visiting),
        "cargo" => cargo_test_uses_workspace(&mut parts),
        command_name => called_function_contains_workspace_test(command_name, root, visiting),
    }
}

fn called_function_contains_workspace_test(
    command_name: &str,
    root: &ParsedShellScript<'_>,
    visiting: &mut Vec<String>,
) -> bool {
...
fn script_contains_workspace_test_with_root(
    ...
    I: Iterator<Item = &'a str>,
{
    ...
    if let Some(script) = split_string {
        ...
        return line_contains_workspace_test(&nested, root, visiting);
    }
    ...
    wrapper_or_command_contains_workspace_test(next, &mut parts, root, visiting)
}

fn shell_wrapper_contains_workspace_test<'a, I>(
    ...
    I: Iterator<Item = &'a str>,
{
    ...
    line_contains_workspace_test(script, root, visiting)
}

fn command_wrapper_contains_workspace_test<'a, I>(
    parts: std::iter::Peekable<I>,
    root: &ParsedShellScript<'_>,
    visiting: &mut Vec<String>,
) -> bool
...
fn exec_wrapper_contains_workspace_test<'a, I>(
    parts: std::iter::Peekable<I>,
    root: &ParsedShellScript<'_>,
    visiting: &mut Vec<String>,
) -> bool
...
fn wrapper_or_command_contains_workspace_test<'a, I>(
    token: &'a str,
    ...
    I: Iterator<Item = &'a str>,
{
    match normalize_command_token(token) {
        "cargo" => cargo_test_uses_workspace(parts),
        "sh" | "bash" => {
            shell_wrapper_contains_workspace_test(parts.by_ref().peekable(), root, visiting)
        }
        "command" => {
            command_wrapper_contains_workspace_test(parts.by_ref().peekable(), root, visiting)
        }
        "exec" => exec_wrapper_contains_workspace_test(parts.by_ref().peekable(), root, visiting),
        "env" => env_wrapper_contains_workspace_test(parts.by_ref().peekable(), root, visiting),
        command_name => called_function_contains_workspace_test(command_name, root, visiting),
    }
}

fn cargo_test_uses_workspace<'a, I>(parts: &mut std::iter::Peekable<I>) -> bool
where
    I: Iterator<Item = &'a str>,
{
