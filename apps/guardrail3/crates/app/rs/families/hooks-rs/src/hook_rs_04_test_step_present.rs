...
fn is_cargo_test_command(command_text: &str) -> bool {
    ...
    if first == "env" {
        ...
        return match normalize_command_token(next) {
            "cargo" => is_cargo_test_invocation(&mut parts),
            _ => false,
        };
    }
...
fn is_cargo_test_invocation<'a, I>(parts: &mut std::iter::Peekable<I>) -> bool
where
