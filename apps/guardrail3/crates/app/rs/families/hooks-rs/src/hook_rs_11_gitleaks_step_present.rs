...
fn script_contains_gitleaks(parsed: &ParsedShellScript<'_>) -> bool {
    parsed
        .executable_lines
        .iter()
        .any(|line| line_contains_gitleaks(line.raw, parsed, &mut Vec::new()))
}

fn line_contains_gitleaks(
    raw: &str,
    ...
    if segments.is_empty() {
        return segment_contains_gitleaks(raw, root, visiting);
    }
    ...
    for segment in segments {
        ...
        if reachable && segment.operator_after != Some("&") && segment.operator_after != Some("|") {
            if segment_contains_gitleaks(&segment.text, root, visiting) {
                return true;
            }
            for substitution in extract_command_substitutions(&segment.text) {
                if line_contains_gitleaks(&substitution, root, visiting) {
                    return true;
                }
            }
        }
...
fn segment_contains_gitleaks(
    ...
    match normalize_command_token(first) {
        "env" => env_wrapper_contains_gitleaks(parts, root, visiting),
        "sh" | "bash" => shell_wrapper_contains_gitleaks(parts, root, visiting),
        "command" => command_wrapper_contains_gitleaks(parts, root, visiting),
        "exec" => exec_wrapper_contains_gitleaks(parts, root, visiting),
        "gitleaks" => gitleaks_invocation(&mut parts),
        command_name => called_function_contains_gitleaks(command_name, root, visiting),
    }
}

fn called_function_contains_gitleaks(
    command_name: &str,
    root: &ParsedShellScript<'_>,
    visiting: &mut Vec<String>,
) -> bool {
    ...
    let found = body_parsed
        .executable_lines
        .iter()
        .any(|line| line_contains_gitleaks(line.raw, root, visiting));
    let _ = visiting.pop();
    found
}

fn env_wrapper_contains_gitleaks<'a, I>(
    ...
    I: Iterator<Item = &'a str>,
{
    ...
    if let Some(script) = split_string {
        ...
        return line_contains_gitleaks(&nested, root, visiting);
    }
    ...
    wrapper_or_command_contains_gitleaks(next, &mut parts, root, visiting)
}

fn shell_wrapper_contains_gitleaks<'a, I>(
    ...
    I: Iterator<Item = &'a str>,
{
    ...
    line_contains_gitleaks(script, root, visiting)
}

fn command_wrapper_contains_gitleaks<'a, I>(
    parts: std::iter::Peekable<I>,
    root: &ParsedShellScript<'_>,
    visiting: &mut Vec<String>,
) -> bool
...
fn exec_wrapper_contains_gitleaks<'a, I>(
    parts: std::iter::Peekable<I>,
    root: &ParsedShellScript<'_>,
    visiting: &mut Vec<String>,
) -> bool
...
fn wrapper_or_command_contains_gitleaks<'a, I>(
    token: &'a str,
    ...
    I: Iterator<Item = &'a str>,
{
    match normalize_command_token(token) {
        "gitleaks" => gitleaks_invocation(parts),
        "sh" | "bash" => shell_wrapper_contains_gitleaks(parts.by_ref().peekable(), root, visiting),
        "command" => command_wrapper_contains_gitleaks(parts.by_ref().peekable(), root, visiting),
        "exec" => exec_wrapper_contains_gitleaks(parts.by_ref().peekable(), root, visiting),
        "env" => env_wrapper_contains_gitleaks(parts.by_ref().peekable(), root, visiting),
        command_name => called_function_contains_gitleaks(command_name, root, visiting),
    }
}

fn gitleaks_invocation<'a, I>(parts: &mut std::iter::Peekable<I>) -> bool
where
    I: Iterator<Item = &'a str>,
{
...
#[cfg(test)]
#[path = "hook_rs_11_gitleaks_step_present_tests/mod.rs"]
