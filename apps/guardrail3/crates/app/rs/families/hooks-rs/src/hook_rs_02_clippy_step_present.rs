...
pub fn check(input: &RustHookCommandInput<'_>, results: &mut Vec<CheckResult>) {
    let found = input
        ...
        .executable_lines
        .iter()
        .any(|line| is_cargo_clippy_command(line.command_text));
    ...
    if found {
        results.push(
            CheckResult {
                ...
                title: "cargo clippy step present".to_owned(),
                message: "Hook runs cargo clippy.".to_owned(),
        ...
        results.push(CheckResult {
            id: ID.to_owned(),
            severity: Severity::Warn,
            title: "cargo clippy step missing".to_owned(),
            message: "Hook does not execute cargo clippy.".to_owned(),
            file: Some(input.rel_path.to_owned()),
            line: None,
            inventory: false,
        });
...
fn is_cargo_clippy_command(command_text: &str) -> bool {
    ...
    if first == "env" {
        ...
        if normalize_command_token(next) != "cargo" {
            return false;
        }
        return is_cargo_clippy_invocation(&mut parts);
    }
    ...
    is_cargo_clippy_invocation(&mut parts)
}

fn is_cargo_clippy_invocation<'a, I>(parts: &mut std::iter::Peekable<I>) -> bool
where
...
#[cfg(test)]
#[path = "hook_rs_02_clippy_step_present_tests/mod.rs"]
