use guardrail3_app_rs_family_garde_assertions::rs_garde_config_05_additional_method_bans as assertions;
...
fn local_missing_additional_ban_only_warns_for_owned_root() {
    ...
    assert_eq!(
        findings.len(),
        2,
        "unexpected RS-GARDE-CONFIG-05 findings: {findings:#?}"
    );
    ...
    assertions::assert_missing(
        ...
        "Missing additional deserialization bans from `disallowed-methods`: serde_json::Deserializer::from_reader. Add these entries to `disallowed-methods` in clippy.toml.",
    );
