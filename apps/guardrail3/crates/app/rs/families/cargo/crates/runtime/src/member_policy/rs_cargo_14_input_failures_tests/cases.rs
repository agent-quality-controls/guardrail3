...
fn undeclared_nested_package_surfaces_explicit_failure() {
    ...
    assert!(
        ...
        "cargo family should ignore undeclared nested packages because shared legality filters them before cargo runs: {results:#?}"
    );
}
...
#[test]
fn malformed_workspace_members_shape_blocks_descendant_standalone_reclassification() {
    let workspace_manifest = format!(
        r#"
            [workspace]
            members = [1]
