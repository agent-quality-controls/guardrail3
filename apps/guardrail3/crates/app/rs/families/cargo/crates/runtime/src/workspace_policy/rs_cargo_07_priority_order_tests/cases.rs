...
#[test]
fn workspace_root_priority_policy_has_same_behavior() {
    let manifest = r#"
        [workspace]
        members = ["crates/api"]
        resolver = "2"
...
#[test]
fn invalid_targeted_lint_level_does_not_emit_clean_inventory() {
    let manifest = format!(
        r#"
            [package]
            name = "helper"
...
#[test]
fn invalid_targeted_lint_priority_type_does_not_emit_clean_inventory() {
    let manifest = format!(
        r#"
            [package]
            name = "helper"
    ...
    .replace(
        r#"unwrap_used = "deny""#,
        r#"unwrap_used = { level = "deny", priority = "banana" }"#,
    );
