...
fn ignores_keys_owned_by_specific_rules() {
    let clippy = build_clippy_toml("library", false, true, "", "")
        .replace("avoid-breaking-exported-api = false", "avoid-breaking-exported-api = true")
        .replace("allow-dbg-in-tests = false", "allow-dbg-in-tests = true")
        .replace("allow-print-in-tests = false", "allow-print-in-tests = true")
        .replace("allow-expect-in-tests = true", "allow-expect-in-tests = false")
        .replace("allow-panic-in-tests = false", "allow-panic-in-tests = true")
        .replace("allow-unwrap-in-tests = false", "allow-unwrap-in-tests = true");
