...
pub fn check(input: &ConfigClippyInput<'_>, results: &mut Vec<CheckResult>) {
    ...
    match integer_setting(parsed, KEY) {
        ...
        IntegerSetting::Value(actual) => results.push(CheckResult::from_parts(
            ...
            format!("Expected {EXCESSIVE_NESTING_THRESHOLD}, got {actual}. Set `{KEY} = {EXCESSIVE_NESTING_THRESHOLD}` in clippy.toml."),
            Some(input.config.rel_path.clone()),
            None,
            false,
        )),
        ...
        IntegerSetting::Missing => results.push(CheckResult::from_parts(
            ...
            Severity::Error,
            format!("{KEY} missing"),
            format!("Add `{KEY} = {EXCESSIVE_NESTING_THRESHOLD}` to clippy.toml."),
            Some(input.config.rel_path.clone()),
            None,
            false,
        )),
