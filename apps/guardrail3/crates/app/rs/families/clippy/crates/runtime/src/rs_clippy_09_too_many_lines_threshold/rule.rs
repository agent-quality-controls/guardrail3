    ...
    match integer_setting(parsed, KEY) {
        IntegerSetting::Value(actual) if actual == TOO_MANY_LINES_THRESHOLD => results.push(
            CheckResult::from_parts(
                ...
                Severity::Info,
                format!("{KEY} correct"),
                format!("{KEY} = {TOO_MANY_LINES_THRESHOLD}"),
        ...
        IntegerSetting::Value(actual) => results.push(CheckResult::from_parts(
            ...
            format!("Expected {TOO_MANY_LINES_THRESHOLD}, got {actual}. Set `{KEY} = {TOO_MANY_LINES_THRESHOLD}` in clippy.toml."),
            Some(input.config.rel_path.clone()),
            None,
            false,
        )),
        ...
        IntegerSetting::Missing => results.push(CheckResult::from_parts(
            ...
            Severity::Error,
            format!("{KEY} missing"),
            format!("Add `{KEY} = {TOO_MANY_LINES_THRESHOLD}` to clippy.toml."),
            Some(input.config.rel_path.clone()),
            None,
            false,
        )),
