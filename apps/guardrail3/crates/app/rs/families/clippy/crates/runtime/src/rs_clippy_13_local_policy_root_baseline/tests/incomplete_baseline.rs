...
fn errors_when_local_policy_root_drops_managed_sections() {
    ...
    assertions::assert_incomplete_baseline(
        ...
        "`apps/backend/clippy.toml` replaces inherited clippy policy but is incomplete. Missing or wrong managed sections: disallowed-macros, disallowed-methods, disallowed-types, thresholds.",
    );
}
