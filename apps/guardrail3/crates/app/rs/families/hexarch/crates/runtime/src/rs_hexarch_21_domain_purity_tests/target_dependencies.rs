...
fn target_dependencies_are_in_scope_for_domain_purity() {
    ...
    let edges = facts
        ...
        .iter()
        .filter(|edge| {
            edge.source_rel_dir == member.rel_dir && edge.kind == EdgeKind::TargetDependency
        })
        .collect();
    ...
    assert_eq!(
        ...
        "target dependencies should be treated as real domain dependencies: {results:#?}"
    );
}
...
fn target_build_dependencies_are_in_scope_for_domain_purity() {
    ...
    let edges = facts
        ...
        .iter()
        .filter(|edge| {
            edge.source_rel_dir == member.rel_dir && edge.kind == EdgeKind::TargetBuildDependency
        })
        .collect();
    let mut results = Vec::new();
    check(
    ...
    let actual_titles = results
        ...
        .collect::<BTreeSet<_>>();
    let expected_titles = [
        "domain crate `api-domain-core` depends on disallowed external crate `prost`".to_owned(),
        "domain crate `api-domain-core` depends on non-pure layer".to_owned(),
    ]
    .into_iter()
    .collect::<BTreeSet<_>>();
    assert_eq!(
        actual_titles, expected_titles,
        "target build dependencies should be treated as real domain dependencies: {results:#?}"
    );
}
