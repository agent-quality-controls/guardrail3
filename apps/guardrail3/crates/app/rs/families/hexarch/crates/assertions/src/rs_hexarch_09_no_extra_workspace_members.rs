...
const RULE_ID: &str = "RS-HEXARCH-09";
...
pub fn assert_expected_rule_results<'a>(
    ...
    assert_eq!(
        actual.len(),
        expected.len(),
        "unexpected RS-HEXARCH-09 results: {actual:#?}"
    );
    ...
    for expected_result in expected {
        ...
        assert!(
            index.is_some(),
            "missing expected RS-HEXARCH-09 result {expected_result:#?}; actual: {actual:#?}"
        );
...
pub fn assert_any_result_contains_title(results: &[CheckResult], needles: &[&str]) {
    ...
    for needle in needles {
        assert!(
            ...
            "missing expected title `{needle}` in RS-HEXARCH-09 results: {errors:#?}"
        );
    }
}
...
pub fn assert_no_error_file_contains(results: &[CheckResult], file: &str) {
    ...
    assert!(
        ...
        "found RS-HEXARCH-09 result in path `{file}`: {errors:#?}"
    );
}
