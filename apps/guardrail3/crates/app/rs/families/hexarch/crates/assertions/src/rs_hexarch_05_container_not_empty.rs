...
const RULE_ID: &str = "RS-HEXARCH-05";
...
pub fn assert_expected_rule_results<'a>(
    ...
    assert_eq!(
        actual.len(),
        expected.len(),
        "unexpected {rule_id} errors: {actual:#?}"
    );
    ...
    for expected_result in expected {
        ...
        assert!(
            index.is_some(),
            "missing expected {rule_id} result {expected_result:#?}; actual: {actual:#?}"
        );
...
pub fn assert_any_result_contains_title(results: &[CheckResult], rule_id: &str, needles: &[&str]) {
    let rule_id = if rule_id.is_empty() { RULE_ID } else { rule_id };
...
pub fn assert_no_error_file_contains(results: &[CheckResult], rule_id: &str, file: &str) {
    let rule_id = if rule_id.is_empty() { RULE_ID } else { rule_id };
    let errors = error_results(results, rule_id);
    assert!(
        errors
            .iter()
            .all(|result| !result.file.as_deref().unwrap_or("").contains(file)),
