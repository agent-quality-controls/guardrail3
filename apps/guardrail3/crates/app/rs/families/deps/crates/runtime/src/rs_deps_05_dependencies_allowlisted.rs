...
pub(super) fn dependency_input<'a>(facts: &'a super::facts::DepsFacts, cargo_rel_path: &str, dep_package_name: &str) -> super::inputs::DependencyEntryDepsInput<'a> {
    let entry = facts.dependency_entries.iter().find(|entry| entry.cargo_rel_path == cargo_rel_path && entry.section_kind == super::facts::DependencySectionKind::Dependencies && entry.dep_package_name == dep_package_name).expect("expected dependency entry facts");
