...
#[test]
fn subtree_scope_still_checks_version_consistency_against_publishable_siblings() {
    let root = temp_root("release-pub-11-validation-scope");
...
"#,
    ...
    assert!(
        results.iter().any(|result| {
            result.id() == "RS-PUB-11"
                && result.file() == Some("crates/consumer/Cargo.toml")
                && result.title() == "consumer: version mismatch with shared"
        }),
        "{results:#?}"
    );
}
