...
pub fn assert_rule_count(results: &[CheckResult], rule_id: &str, expected: usize) {
    let actual = rule_results(results, rule_id);
    assert_eq!(
        actual.len(),
        expected,
        "unexpected {rule_id} findings: {actual:#?}"
    );
}
...
macro_rules! define_rule_assertions {
    ($rule_id:literal) => {
        ...
        pub fn assert_rule_count(results: &[CheckResult], expected: usize) {
            crate::common::assert_rule_count(results, RULE_ID, expected);
        }
