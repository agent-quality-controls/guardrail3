...
pub fn assert_attacks_unwrap_and_expect_calls_across_real_owned_files_with_exact_metadata(
    results: &[CheckResult],
    backend_rel: &str,
    worker_rel: &str,
    backend_unwrap_line: usize,
    backend_expect_line: usize,
    worker_expect_line: usize,
    worker_unwrap_line: usize,
) {
    ...
    assert_value_eq(
        ...
        vec![
            Finding {
                ...
                title: ".unwrap() usage",
                message: "`.unwrap()` found: fn unwrap_probe() { let _ = Some(1).unwrap(); }.",
                file: Some(backend_rel),
                line: Some(backend_unwrap_line),
                inventory: false,
            },
            Finding {
                id: ID,
                severity: Severity::Warn,
                title: ".expect() usage",
                message: "`.expect()` found: fn expect_probe() { let _ = Some(1).expect(\"backend\"); }."
                    ,
                file: Some(backend_rel),
                line: Some(backend_expect_line),
                inventory: false,
            },
            Finding {
                id: ID,
                severity: Severity::Warn,
                title: ".expect() usage",
                message: "`.expect()` found: let _ = Some(1).expect(\"queue\");.",
                file: Some(worker_rel),
                line: Some(worker_expect_line),
                inventory: false,
            },
            Finding {
                id: ID,
                severity: Severity::Warn,
                title: ".unwrap() usage",
                message: "`.unwrap()` found: let _ = Some(1).unwrap();.",
                file: Some(worker_rel),
                line: Some(worker_unwrap_line),
                inventory: false,
            },
        ],
    );
