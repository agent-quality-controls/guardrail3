...
use guardrail3_app_rs_family_code_assertions::rs_code_32_test_expect_message_quality::{Severity, 
    ...
    assert_findings(
        ...
        &[
            RuleFinding {
                ...
                message: "Test `expect(...)` message must be a useful string literal or `concat!` of string literals, not an indirect expression: `let _ = Some(1).expect(message);`.",
                file: Some(cfg_rel),
                line: Some(cfg_line),
                inventory: false,
            },
            RuleFinding {
                ...
                message: "Test `expect(...)` message must explain the failure clearly. Weak message `ok` found in `let _ = Some(1).expect(\"ok\");`.",
                file: Some(test_rel),
                line: Some(test_line),
                inventory: false,
            },
