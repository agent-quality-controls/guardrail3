...
fn attacks_always_true_cfg_attr_bypasses_across_multiple_owned_files_with_exact_metadata() {
    ...
    let backend_top_line = backend_new
        .lines()
        .position(|line| line.contains("#[cfg_attr(all(), allow(clippy::unwrap_used))]")).map(|index| index + 1).unwrap_or_default();
    let backend_nested_line = backend_new
        .lines()
        .position(|line| line.contains("#[cfg_attr(any(unix, windows), allow(clippy::panic))]")).map(|index| index + 1).unwrap_or_default();
