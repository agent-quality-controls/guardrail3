    ...
    assert_value_eq(
        ...
        vec![
            Finding {
                ...
                message: "Direct `use std::fs` import found: `use std::fs;`. Route filesystem access through a dedicated `fs` module or crate instead of using `std::fs` directly.",
            ...
            Finding {
                ...
                message: "Direct `std::fs::*` call found: `let _ = std::fs::read_to_string(\"backend.txt\");`. Route filesystem access through a dedicated `fs` module or crate instead of using `std::fs` directly.",
                file: Some(backend_rel),
            ...
            Finding {
                ...
                severity: Severity::Error,
                title: "direct std::fs import",
                message: "Direct `use std::fs` import found: `use std::{fs, io}; fn fs_call_probe() { let _ = std::fs::read_to_string(\"jobs.txt\"); }`. Route filesystem access through a dedicated `fs` module or crate instead of using `std::fs` directly.",
...
pub fn assert_prefers_import_hit_when_import_and_call_share_one_line(
    ...
    assert_value_eq(
        ...
        vec![Finding {
            ...
            severity: Severity::Error,
            title: "direct std::fs import",
            message: "Direct `use std::fs` import found: `use std::fs; fn same_line_probe() { let _ = std::fs::read_to_string(\"same-line.txt\"); }`. Route filesystem access through a dedicated `fs` module or crate instead of using `std::fs` directly.",
            file: Some(rel),
            line: Some(line),
            inventory: false,
        }],
