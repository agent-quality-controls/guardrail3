...
fn inventories_large_traits_in_real_library_profile_files() {
    ...
    let warn_line = mutated
        .lines()
        .position(|line| line.contains("pub trait SharedSurface")).map(|index| index + 1).unwrap_or_default();
    let error_line = mutated
        .lines()
        .position(|line| line.contains("pub trait OversizedSurface")).map(|index| index + 1).unwrap_or_default();
