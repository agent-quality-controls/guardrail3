...
const ID: &str = "RS-ARCH-13";

pub fn check(input: &TopologyIssueInput<'_>, results: &mut Vec<CheckResult>) {
    let ArchTopologyIssueKind::WorkspaceMemberPathEscapesRoot {
        workspace_root_rel,
        member_pattern,
    } = &input.issue.kind
    else {
...
#[cfg(test)]
#[path = "rs_arch_13_member_paths_must_not_escape_root_tests/mod.rs"]
mod rs_arch_13_member_paths_must_not_escape_root_tests;
