...
fn undeclared_workspace_child_is_reported() {
    ...
    let result = results
        ...
        .expect("expected missing-child RS-ARCH-12 result");
...
fn extra_workspace_member_is_reported() {
    ...
    let result = crate::check_test_tree(&tree)
        .into_iter()
        .find(|result| {
            result.id() == "RS-ARCH-12" && result.file() == Some("apps/backend/Cargo.toml")
        })
        .expect("expected extra-member RS-ARCH-12 result");
...
fn exact_match_stays_clean() {
    ...
    assert!(
        crate::check_test_tree(&tree)
            .into_iter()
            .all(|result| result.id() != "RS-ARCH-12")
    );
}
...
fn missing_and_extra_are_both_reported() {
    ...
    let results = crate::check_test_tree(&tree);
    assert_eq!(
        results.iter().filter(|result| result.id() == "RS-ARCH-12").count(),
        2
    );
