...
#[test]
fn path_dependency_is_inventoried() {
    let tree = project_tree(
        ...
        vec![
            ...
            (
                ...
                "[package]\nname = \"api-app\"\n[dependencies]\napi-domain = { path = \"../domain\" }\n",
            ),
            ("apps/api/crates/domain/Cargo.toml", "[package]\nname = \"api-domain\"\n"),
        ],
    );

    let facts = dependency_facts(&tree);
    let edge = find_edge(&facts, "apps/api/crates/app", "api-domain", EdgeKind::Dependency);
    let mut results = Vec::new();
    check(&DependencyEdgeHexarchInput::new(edge), &mut results);

    assert_eq!(results.len(), 1, "expected one inventory result: {results:#?}");
