...
fn inventories_too_many_lines_threshold_when_correct() {
    let tree = root_workspace_tree("too-many-lines-threshold = 75");
    let facts = collected_facts(&tree);
    let input = ConfigClippyInput::new(facts.allowed_configs.first().expect("config"));
    let mut results = Vec::new();
    check(&input, &mut results);
    assert!(results.iter().any(|result| {
        result.id == "RS-CLIPPY-09"
            && result.inventory
            && result.severity == Severity::Info
            && result.title == "too-many-lines-threshold correct"
            && result.message == "too-many-lines-threshold = 75"
    }));
}
...
fn errors_when_too_many_lines_threshold_is_wrong() {
    ...
    assert!(results.iter().any(|result| {
        result.id == "RS-CLIPPY-09"
            ...
            && result.title == "too-many-lines-threshold wrong value"
...
fn errors_when_too_many_lines_threshold_is_missing() {
    ...
    assert!(results.iter().any(|result| {
        result.id == "RS-CLIPPY-09"
            && result.severity == Severity::Error
            && result.title == "too-many-lines-threshold missing"
            && result.message == "Expected too-many-lines-threshold = 75."
    }));
}
