...
pub fn check_i18n(fs: &dyn FileSystem, path: &Path, results: &mut Vec<CheckResult>) {
    ...
    results.push(
        CheckResult {
            ...
            title: format!("i18n library detected: {lib_name}"),
            message: format!("{lib_name} found in dependencies. Checking i18n completeness."),
    ...
    let Some(msg_dir) = found_dir else {
        results.push(CheckResult {
            ...
            title: "No i18n message directory found".to_owned(),
            message: format!(
    ...
    if missing_report.is_empty() {
        results.push(
            CheckResult {
                id: "T-TOOL-12".to_owned(),
                severity: Severity::Info,
                title: "i18n locale files consistent".to_owned(),
                message: format!(
