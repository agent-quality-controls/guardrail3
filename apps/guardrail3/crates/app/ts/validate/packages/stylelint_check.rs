...
pub fn check_stylelint(fs: &dyn FileSystem, path: &Path, results: &mut Vec<CheckResult>) {
    ...
    "T-STYL-01".to_owned(),
    Severity::Error,
    "Stylelint config not found".to_owned(),
    "No stylelint config file found. Create .stylelintrc.mjs with \
    ...
    results.push(
        CheckResult::from_parts(
            "T-STYL-01".to_owned(),
            Severity::Info,
            "Stylelint config found".to_owned(),
            "Stylelint configuration file found.".to_owned(),
    ...
    if content.contains("@double-great/stylelint-a11y") {
        results.push(
            CheckResult::from_parts(
                "T-STYL-04".to_owned(),
                Severity::Info,
                "Stylelint a11y plugin configured".to_owned(),
                "@double-great/stylelint-a11y found in stylelint config.".to_owned(),
                Some(file_display.clone()),
                None,
                false,
            )
    ...
    if missing.is_empty() {
        results.push(
            CheckResult::from_parts(
                "T-STYL-05".to_owned(),
                Severity::Info,
                "All stylelint a11y rules enabled".to_owned(),
                format!(
    ...
    if missing_exceptions.is_empty() {
        results.push(
            CheckResult::from_parts(
                "T-STYL-06".to_owned(),
                Severity::Info,
                "Stylelint architecture exceptions configured".to_owned(),
                "Architecture exceptions (disabled rules) are correctly configured."
                    .to_owned(),
                Some(file_display.clone()),
                None,
                false,
    ...
    if content.contains(expected) {
        results.push(
            CheckResult::from_parts(
                check_id.to_owned(),
                Severity::Info,
                format!("{expected} configured"),
                format!("{expected} found in stylelint extends."),
                Some(file.to_owned()),
                None,
                false,
            )
