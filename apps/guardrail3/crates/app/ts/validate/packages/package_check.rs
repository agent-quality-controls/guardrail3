    ...
    if is_private {
        results.push(
            CheckResult::from_parts(
                "T-PKG-01".to_owned(),
                Severity::Info,
                "`private` field set to `true` in `package.json`".to_owned(),
                "`\"private\": true` found in `package.json`. This prevents accidental publication to npm."
                    .to_owned(),
                ...
                if !["zod", "@eslint/js"].contains(&key.as_str()) {
                    results.push(
                        CheckResult::from_parts(
                            "T16".to_owned(),
                            Severity::Info,
                            format!("Extra pnpm override: `{key}`"),
                            format!(
        ...
        Some(script) if script.contains("only-allow pnpm") => {
            results.push(
                CheckResult::from_parts(
                    "T55".to_owned(),
                    Severity::Info,
                    "`preinstall` script enforces pnpm".to_owned(),
                    "`preinstall` script contains `only-allow pnpm`. This prevents accidentally running \
    ...
    if has_pnpm_engine {
        results.push(
            CheckResult::from_parts(
                "T-PKG-04".to_owned(),
                Severity::Info,
                "`engines.pnpm` version constraint set".to_owned(),
                "pnpm version constraint found in `engines` field.".to_owned(),
                Some(pkg_path.display().to_string()),
                None,
                false,
            )
