...
pub fn check_eslint_config(
    ...
    "T1".to_owned(),
    Severity::Error,
    "ESLint config `eslint.config.mjs` not found".to_owned(),
    "ESLint enforces code quality rules (no-unused-vars, naming conventions, import order, \
    ...
    for eslint_path in eslint_configs {
        results.push(
            CheckResult::from_parts(
                "T1".to_owned(),
                Severity::Info,
                "ESLint config exists".to_owned(),
                format!("ESLint flat config found: `{}`.", eslint_path.display()),
    ...
    if config.has_regexp_ban() {
        results.push(
            CheckResult::from_parts(
                "T-ESLP-15".to_owned(),
                Severity::Info,
                "RegExp ban configured in ESLint".to_owned(),
                "RegExp is banned via ESLint rules. Use Zod schemas for validation, \
        ...
        .any(|p| p.contains("strictTypeChecked"))
    {
        results.push(
            CheckResult::from_parts(
                "T-ESLP-13".to_owned(),
                Severity::Info,
                "ESLint strictTypeChecked preset configured".to_owned(),
        ...
        .any(|p| p.contains("stylisticTypeChecked"))
    {
        results.push(
            CheckResult::from_parts(
                "T-ESLP-14".to_owned(),
                Severity::Info,
                "ESLint stylisticTypeChecked preset configured".to_owned(),
    ...
    if config.has_route_wrappers() {
        results.push(
            CheckResult::from_parts(
                "T50".to_owned(),
                Severity::Info,
                "Route wrapper enforcement configured".to_owned(),
