...
pub struct EslintConfig {
    ...
    has_process_env_ban: bool,
    /// Whether route wrapper enforcement is configured.
    has_route_wrappers: bool,
...
impl EslintConfig {
    ...
    #[must_use]
    pub const fn rules(&self) -> &BTreeMap<String, RuleConfig> {
        &self.rules
    }
    ...
    #[must_use]
    pub const fn has_process_env_ban(&self) -> bool {
        self.has_process_env_ban
    }

    #[must_use]
    pub const fn has_route_wrappers(&self) -> bool {
        self.has_route_wrappers
    }

    #[must_use]
    pub const fn has_regexp_ban(&self) -> bool {
        self.has_regexp_ban
    }
...
impl RuleConfig {
    #[must_use]
    pub fn new(severity: String, numeric_value: Option<u32>, is_test_override: bool) -> Self {
        Self {
            severity,
            numeric_value,
            is_test_override,
        ...
        if child.kind() == "pair" {
            ...
            if should_insert {
                let _ = config.rules.insert(
                    rule_name,
                    RuleConfig::new(severity, numeric_value, is_test),
                );
            }
        }
    }
