    ...
    if missing.is_empty() {
        results.push(
            CheckResult::from_parts(
                id.to_owned(),
                Severity::Info,
                format!("{group_name} rules configured"),
                format!("All expected {group_name} rules found in ESLint config."),
    ...
    if all_present {
        results.push(
            CheckResult::from_parts(
                id.to_owned(),
                Severity::Info,
                format!("{plugin_name} config import present"),
                format!("{plugin_name} flat config import found in ESLint config."),
                Some(eslint_path.display().to_string()),
                None,
                false,
            )
...
pub fn check_core_plugins(
    ...
    "T-ESLP-10".to_owned(),
    Severity::Warn,
    "naming-convention missing selector config".to_owned(),
    "@typescript-eslint/naming-convention is present but no 'selector' \
    ...
    if has_unicorn && has_config {
        results.push(
            CheckResult::from_parts(
                "T-ESLP-01".to_owned(),
                Severity::Info,
                "unicorn config import present".to_owned(),
                "unicorn flat config import found in ESLint config.".to_owned(),
                Some(eslint_path.display().to_string()),
                None,
                false,
            )
...
fn check_test_relaxations(
    ...
    "T-ESLP-11".to_owned(),
    Severity::Error,
    "Test file relaxation section missing".to_owned(),
    "No test file override section found in ESLint config. \
    ...
    if has_a11y && has_strict {
        results.push(
            CheckResult::from_parts(
                "T-ESLP-07".to_owned(),
                Severity::Info,
                "jsx-a11y strict config present".to_owned(),
                "jsx-a11y strict configuration found in ESLint config.".to_owned(),
                Some(eslint_path.display().to_string()),
                None,
                false,
            )
    ...
    if config.rules().contains_key(rule) {
        results.push(
            CheckResult::from_parts(
                "T-ESLP-08".to_owned(),
                Severity::Info,
                format!("ESLint rule `{rule}` configured"),
                format!("`{rule}` found in ESLint config."),
                Some(eslint_path.display().to_string()),
                None,
                false,
            )
    ...
    if has_plugin {
        results.push(
            CheckResult::from_parts(
                "T-ESLP-12".to_owned(),
                Severity::Info,
                "tailwind-ban plugin configured".to_owned(),
                "tailwind-ban plugin found in ESLint config.".to_owned(),
                Some(eslint_path.display().to_string()),
                None,
                false,
            )
