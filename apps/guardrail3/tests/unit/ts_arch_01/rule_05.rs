...
fn empty_domain() {
    ...
    assert_eq!(
        empty.len(),
        1,
        "expected 1 empty-container error, got {}: {empty:#?}",
        empty.len()
    );
    assert!(
        empty[0].title.contains("domain"),
        "expected domain in title"
    );
    assert!(
        empty[0].message.contains("is empty"),
        "expected 'is empty' in message"
    );
...
fn empty_application() {
    ...
    assert!(
        empty[0].title.contains("application"),
        "expected application in title"
    );
    assert_standard_admin(&empty, &errors);
}
...
fn empty_adapters_inbound() {
    ...
    assert!(
        empty[0].title.contains("adapters") && empty[0].title.contains("inbound"),
...
fn empty_ports_outbound() {
    ...
    assert!(
        empty[0].title.contains("ports") && empty[0].title.contains("outbound"),
...
fn all_containers_empty_both_apps() {
    ...
    assert_eq!(
        empty.len(),
        12,
        "expected 12 empty-container errors (6 per app), got {}: {empty:#?}",
        empty.len()
    );
    for app in TS_SERVICE_APPS {
        assert!(
...
fn gitkeep_satisfies_non_empty() {
    ...
    let errors = arch_errors(&results);
    let empty = empty_errors(&errors);
    let domain_empty: Vec<_> = empty
        .iter()
        .filter(|e| e.title.contains("domain"))
        .collect();
    assert_eq!(
        domain_empty.len(),
        0,
        ".gitkeep should satisfy non-empty for domain/, got: {domain_empty:#?}"
    );
}
...
fn gitkeep_in_all_containers() {
    ...
    for suffix in CONTAINER_SUFFIXES {
        ...
        write_file(
            tmp.path(),
            &format!("apps/admin/src/modules/{suffix}/.gitkeep"),
            "",
        );
    }
    let results = run_check(tmp.path());
    ...
    assert_eq!(
        admin_empty.len(),
        0,
        ".gitkeep should satisfy non-empty for all containers, got: {admin_empty:#?}"
    );
}
...
fn subdir_satisfies_non_empty() {
    ...
    assert_eq!(
        empty.len(),
        0,
        "golden (subdirs present) should have 0 empty-container errors, got: {empty:#?}"
    );
}
...
fn files_but_no_subdirs() {
    ...
    assert_eq!(
        domain_empty.len(),
        1,
        "expected 1 empty-container error with file listing, got: {domain_empty:#?}"
    );
...
fn multiple_files_but_no_subdirs() {
    ...
    assert_eq!(
        domain_empty.len(),
        1,
        "expected 1 empty-container error listing all files"
    );
    assert!(
        domain_empty[0].message.contains("index.ts"),
        "expected index.ts"
    );
    assert!(
        domain_empty[0].message.contains("types.ts"),
        "expected types.ts"
    );
    ...
    let inner_empty: Vec<_> = empty
        ...
        .filter(|e| {
            e.file
                .as_deref()
                .unwrap_or("")
                .contains("payments/modules/domain")
        })
        .collect();
    assert_eq!(
        inner_empty.len(),
        1,
        "expected 1 empty error in payments inner hex domain/, got: {inner_empty:#?}"
    );
    assert!(
        inner_empty[0].title.contains("portal"),
...
fn all_inner_hex_containers_empty() {
    ...
    assert_eq!(
        inner_empty.len(),
        12,
        "expected 12 empty inner hex container errors, got {}: {inner_empty:#?}",
        inner_empty.len()
    );
    ...
    assert_eq!(
        admin_empty.len(),
        0,
        "admin should have 0 empty errors: {admin_empty:#?}"
    );
    assert_no_rust_apps(&errors);
    assert_no_packages(&errors);
    assert_no_landing(&errors);
...
fn no_double_fire_files_but_no_subdirs() {
    ...
    assert_eq!(
        domain_empty.len(),
        1,
        "expected 1 empty-container error: {domain_empty:#?}"
    );
    assert!(
        domain_empty[0].message.contains("stray.ts"),
    ...
    let loose: Vec<_> = errors
        .iter()
        .filter(|e| {
            e.title.contains("loose files") && e.file.as_deref().unwrap_or("").contains("domain")
        })
        .collect();
    assert_eq!(
        loose.len(),
        0,
        "should NOT have loose-files error (double-fire prevention), got: {loose:#?}"
    );
    assert_no_rust_apps(&errors);
    assert_no_packages(&errors);
    assert_no_landing(&errors);
...
fn subdirs_plus_loose_files_only_loose_fires() {
    ...
    assert_eq!(
        domain_empty.len(),
        0,
        "should NOT have empty-container error (has subdirs), got: {domain_empty:#?}"
    );
...
fn gitkeep_plus_loose_files_no_subdirs() {
    ...
    assert_eq!(
        domain_empty.len(),
        0,
        ".gitkeep should prevent empty-container error, got: {domain_empty:#?}"
    );
    ...
    assert_eq!(
        loose.len(),
        1,
        "expected 1 loose-files error for stray.ts, got: {loose:#?}"
    );
}
...
fn gitkeep_as_directory_does_not_satisfy() {
    ...
    std::fs::create_dir_all(tmp.path().join("apps/admin/src/modules/domain/.gitkeep"))
        .expect("mkdir");
    let results = run_check(tmp.path());
    let errors = arch_errors(&results);
    let empty = empty_errors(&errors);
    ...
    assert_eq!(
        domain_empty.len(),
        0,
        ".gitkeep dir is a subdir → non-empty, got: {domain_empty:#?}"
    );
}
...
fn unicode_lookalike_gitkeep_does_not_satisfy() {
    ...
    assert_eq!(
        domain_empty.len(),
        1,
        "unicode lookalike should NOT satisfy non-empty, got: {domain_empty:#?}"
    );
}
...
fn dangling_symlink_as_only_content() {
    ...
    assert_eq!(
        domain_empty.len(),
        1,
        "expected 1 empty-container error for dangling symlink, got: {domain_empty:#?}"
    );
}
...
fn hidden_file_as_sole_content() {
    ...
    assert_eq!(
        domain_empty.len(),
        1,
        "expected 1 empty-container error for .DS_Store, got: {domain_empty:#?}"
    );
    assert!(
        domain_empty[0].message.contains(".DS_Store"),
...
fn empty_in_one_app_valid_in_another() {
    ...
    assert_eq!(
        admin_empty.len(),
        1,
        "expected 1 admin empty error: {admin_empty:#?}"
    );
    assert_eq!(
        portal_empty.len(),
        0,
        "portal should have 0 empty errors: {portal_empty:#?}"
    );
    assert_no_rust_apps(&errors);
    assert_no_packages(&errors);
    assert_no_landing(&errors);
...
fn gitkeep_with_content_satisfies() {
    ...
    assert_eq!(
        domain_empty.len(),
        0,
        ".gitkeep with content should satisfy non-empty, got: {domain_empty:#?}"
    );
}
...
fn near_miss_gitkeep_does_not_satisfy() {
    ...
    assert!(
        domain_empty[0].message.contains(".git_keep"),
...
fn empty_container_symlink_to_empty_dir() {
    ...
    assert_eq!(
        domain_empty.len(),
        1,
        "expected 1 empty-container error for symlink to empty dir, got: {domain_empty:#?}"
    );
}
...
fn container_no_read_permission() {
    ...
    assert_eq!(
        domain_empty.len(),
        1,
        "unreadable domain/ should produce empty-container error, got: {domain_empty:#?}"
    );
    assert_standard_admin(&domain_empty, &errors);
}
...
fn idempotent_results() {
    ...
    assert!(
        empty_1.len() > 0,
        "precondition: expected empty-container errors"
    );
...
fn per_app_attribution() {
    ...
    assert_eq!(
        empty.len(),
        2,
        "expected 2 empty-container errors (1 per app), got {}: {empty:#?}",
        empty.len()
    );
    for app in TS_SERVICE_APPS {
        assert!(
...
fn different_breakage_per_app() {
    ...
    write_file(
        tmp.path(),
        "apps/portal/src/modules/domain/stray.ts",
        "// stray",
    );
    let results = run_check(tmp.path());
    let errors = arch_errors(&results);
    ...
    assert_eq!(
        portal_empty.len(),
        1,
        "expected 1 portal empty error: {portal_empty:#?}"
    );
    // admin message: "is empty"
    assert!(
        admin_empty[0].message.contains("is empty"),
        "admin should say 'is empty'"
    );
    // portal message: "contains files (stray.ts) but no subdirectories"
    assert!(
        portal_empty[0].message.contains("stray.ts"),
        "portal should list stray.ts"
    );
    assert!(
        portal_empty[0].message.contains("but no subdirectories"),
        "portal should say 'but no subdirectories'"
    );
    assert_standard(&empty, &errors);
}
...
fn new_app_gets_checked() {
    ...
    for suffix in CONTAINER_SUFFIXES {
        std::fs::create_dir_all(
            tmp.path()
                .join(format!("apps/dashboard/src/modules/{suffix}")),
        )
        .expect("mkdir");
    }
    ...
    let errors = arch_errors(&results);
    let empty = empty_errors(&errors);
    let dashboard_empty: Vec<_> = empty
        .iter()
    ...
    assert_eq!(
        dashboard_empty.len(),
        6,
        "expected 6 empty-container errors for dashboard, got: {dashboard_empty:#?}"
    );
...
fn file_replacing_subdir() {
    ...
    write_file(
        tmp.path(),
        "apps/admin/src/modules/domain/types",
        "not a directory",
    );
    let results = run_check(tmp.path());
    let errors = arch_errors(&results);
...
fn maximally_complex_empty_container() {
    ...
    assert_eq!(
        domain_empty.len(),
        1,
        "expected 1 empty-container error listing all junk: {domain_empty:#?}"
    );
    assert!(
        domain_empty[0].message.contains(".DS_Store"),
        "expected .DS_Store"
    ...
    assert!(
        domain_empty[0].message.contains("README.md"),
        "expected README.md"
    );
    assert!(domain_empty[0].message.contains(".env"), "expected .env");
    assert_standard_admin(&domain_empty, &errors);
